...
export interface PreparationDropdownProps {
    ...
    activeGroups: string[];
    onAddPreparation: (preparationId: string) => void;
    disabled?: boolean;
}

const PreparationDropdown: React.FC<PreparationDropdownProps> = ({
    registry,
    activeGroups,
    onAddPreparation,
    disabled = false,
}) => {
    const availablePreparations = registry.filter(
        (preparation) => !activeGroups.includes(preparation.id)
    );
    ...
    return (
        <div className="relative inline-block">
            <select
                ...
                disabled={disabled}
                onChange={(event) => {
                    const preparationId = event.target.value;

                    if (preparationId) {
                        onAddPreparation(preparationId);
                    }
                }}
            ...
            <Plus
                className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-emerald-700"
            />
        </div>
    );
};
