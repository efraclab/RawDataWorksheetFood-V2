...
import { calculatePhenol } from "../calculation";
...
const step = (preparation: SamplePreparationPhenol | null, name: string) =>
  preparation?.steps.find(
    (item) => item.name.trim().toLowerCase() === name.trim().toLowerCase(),
  );

interface Props {
  calculation: CalculationPhenol;
...
export default function CalculationDetailPhenol({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const update = <K extends keyof CalculationPhenol>(
    field: K,
    value: CalculationPhenol[K],
  ...
  const runCalculation = () => {
    ...
    const result = calculatePhenol(values);
    onUpdate({
      ...calculation,
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        ...
        >
          {calculation.label} · Phenolic Compounds (as C6H5OH)
        </button>
