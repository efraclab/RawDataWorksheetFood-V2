...
import type { SamplePreparationMagnesium, SamplePreparationMagnesiumStep } from "../models/SamplePreparationMagnesium";
import type { CalculationMagnesium } from "../models/CalculationMagnesium";
import { createCalculationMagnesium, createSamplePreparationMagnesium, restoreCalculationMagnesium } from "../factory";
import MagnesiumCalculationSection from "./MagnesiumCalculationSection";

type ModuleData = {
  samplePreparations: SamplePreparationMagnesium[];
  files: PreparationAttachedFile[];
  calculations: CalculationMagnesium[];
  completed: boolean;
  completedAt: string | null;
};

export interface MagnesiumPreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationMagnesium => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationMagnesium>;
  const steps = Array.isArray(source.steps) ? source.steps : createSamplePreparationMagnesium(index).steps;
  return {
    ...createSamplePreparationMagnesium(index),
    ...source,
    id: typeof source.id === "number" ? source.id : index + 1,
    label: typeof source.label === "string" ? source.label : `Sample Preparation ${index + 1}`,
    steps: steps as SamplePreparationMagnesiumStep[],
  };
};

const MagnesiumPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, MagnesiumPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationMagnesium[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationMagnesium[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationMagnesium(items.length)]);
      notify("Magnesium sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationMagnesiumStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("Magnesium preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("Magnesium preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationMagnesium[], preparations: SamplePreparationMagnesium[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationMagnesium(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredCalculations = (value.calculations ?? [])
          ...
          .map((item) => {
            ...
            return restoreCalculationMagnesium(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">Magnesium (as Mg) Analysis</h2>
                <p className="text-sm text-emerald-600">Water Laboratory • Magnesium Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for Magnesium (as Mg)"
            ...
            <div className="mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white">
              <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
                <div>
                  ...
                  <div className="text-xs text-emerald-100">Magnesium (as Mg) inputs</div>
                </div>
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach Magnesium (as Mg) Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
            <PreparationCompleteSection preparationName="Magnesium (as Mg)" isCompleted={completed} isLocked={locked} completedAt={completedAt} canUnlockPreparation={canUnlockPreparation} onComplete={() => setShowComplete(true)} onUnlock={() => setShowUnlock(true)} />
          </div>
          {completed && <MagnesiumCalculationSection calculations={calculations} samplePreparations={samplePreparations} canEditCalculations={canEditCalculations} onAdd={() => {
            setCalculations((items) => [...items, { ...createCalculationMagnesium(items.length), selectedSamplePreparationLabel: samplePreparations[0]?.label ?? null }]);
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="Magnesium (as Mg)" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
MagnesiumPreparationModule.displayName = "MagnesiumPreparationModule";
export default MagnesiumPreparationModule;
