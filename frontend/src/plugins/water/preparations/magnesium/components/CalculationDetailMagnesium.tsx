...
import type { SamplePreparationMagnesium } from "../models/SamplePreparationMagnesium";
import { calculateMagnesium } from "../calculation";
...
const step = (preparation: SamplePreparationMagnesium | null, name: string) =>
  preparation?.steps.find((item) => item.name.trim().toLowerCase() === name.trim().toLowerCase());

interface Props {
  calculation: CalculationMagnesium;
  samplePreparations: SamplePreparationMagnesium[];
  onUpdate: (calculation: CalculationMagnesium) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailMagnesium({ calculation, samplePreparations, onUpdate, onRemove, isLocked }: Props) {
  ...
  const values = useMemo(() => ({ sample: step(selected, "Volume of sample")?.value1 ?? "", df: step(selected, "Dilution factor")?.value1 ?? "", edta: step(selected, "Volume of EDTA")?.value1 ?? "", strength: step(selected, "Strength of EDTA")?.value1 ?? "" }), [selected]);
  const errors = ["sample", "df", "edta", "strength"].map((key) =>
  ...
  const update = <K extends keyof CalculationMagnesium>(field: K, value: CalculationMagnesium[K]) => onUpdate({ ...calculation, [field]: value });
  const runCalculation = () => {
    if (!selected || errors.length > 0) return;
    const result = calculateMagnesium(values);
    onUpdate({ ...calculation, selectedSamplePreparationLabel: selected.label, calculationResult: result.success ? result.result : null, calculationResultSecondary: result.success ? result.resultSecondary : null, calculationResultTertiary: result.success ? result.resultTertiary : null, calculationResultUnit: "mg/L" });
  };
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        <button type="button" onClick={() => setExpanded((value) => !value)} className="font-semibold focus:outline-none focus:ring-0 focus:ring-0">{calculation.label} · Magnesium</button>
          ...
          {selected && (
            <>
              <div className="space-y-2 rounded-lg border border-emerald-200 bg-slate-50 p-4 text-center font-mono text-xs leading-6">
                Total Hardness (CaCO3) = (Volume of EDTA × Strength × 100 × 1000 × DF) / Volume of sample<br />Calcium Hardness (CaCO3) = (Volume of EDTA × Strength × 100 × 1000 × DF) / Volume of sample<br />Magnesium (Mg) = (Total Hardness − Calcium Hardness) × 0.243
              </div>
              {errors.length > 0 && (
