...
import type { SamplePreparationSulphide } from "../models/SamplePreparationSulphide";
import { calculateSulphide } from "../calculation";
...
const step = (preparation: SamplePreparationSulphide | null, name: string) =>
  preparation?.steps.find(
    (item) => item.name.trim().toLowerCase() === name.trim().toLowerCase(),
  );

interface Props {
  calculation: CalculationSulphide;
  samplePreparations: SamplePreparationSulphide[];
  onUpdate: (calculation: CalculationSulphide) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailSulphide({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const update = <K extends keyof CalculationSulphide>(
    field: K,
    value: CalculationSulphide[K],
  ...
  const runCalculation = () => {
    ...
    const result = calculateSulphide(values);
    onUpdate({
      ...calculation,
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        ...
        >
          {calculation.label} · Sulphide (as H2S)
        </button>
