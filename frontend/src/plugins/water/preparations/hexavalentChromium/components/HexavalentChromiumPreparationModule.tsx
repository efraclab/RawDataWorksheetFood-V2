...
import type { SamplePreparationHexavalentChromium, SamplePreparationHexavalentChromiumStep } from "../models/SamplePreparationHexavalentChromium";
import type { CalculationHexavalentChromium } from "../models/CalculationHexavalentChromium";
import { createCalculationHexavalentChromium, createSamplePreparationHexavalentChromium, restoreCalculationHexavalentChromium } from "../factory";
import HexavalentChromiumCalculationSection from "./HexavalentChromiumCalculationSection";

type ModuleData = {
  samplePreparations: SamplePreparationHexavalentChromium[];
  files: PreparationAttachedFile[];
  calculations: CalculationHexavalentChromium[];
  completed: boolean;
  completedAt: string | null;
};

export interface HexavalentChromiumPreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationHexavalentChromium => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationHexavalentChromium>;
  const steps = Array.isArray(source.steps) ? source.steps : createSamplePreparationHexavalentChromium(index).steps;
  return {
    ...createSamplePreparationHexavalentChromium(index),
    ...source,
    id: typeof source.id === "number" ? source.id : index + 1,
    label: typeof source.label === "string" ? source.label : `Sample Preparation ${index + 1}`,
    steps: steps as SamplePreparationHexavalentChromiumStep[],
  };
};

const HexavalentChromiumPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, HexavalentChromiumPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationHexavalentChromium[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationHexavalentChromium[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationHexavalentChromium(items.length)]);
      notify("HexavalentChromium sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationHexavalentChromiumStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("HexavalentChromium preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("HexavalentChromium preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationHexavalentChromium[], preparations: SamplePreparationHexavalentChromium[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationHexavalentChromium(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "hexavalentchromium")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "hexavalentchromium")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationHexavalentChromium(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">Hexavalent Chromium (as Cr6+) Analysis</h2>
                <p className="text-sm text-emerald-600">Water Laboratory • HexavalentChromium Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for Hexavalent Chromium (as Cr6+)"
            ...
            <div className="mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white">
              <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
                <div>
                  ...
                  <div className="text-xs text-emerald-100">Hexavalent Chromium (as Cr6+) inputs</div>
                </div>
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach Hexavalent Chromium (as Cr6+) Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
            <PreparationCompleteSection preparationName="Hexavalent Chromium (as Cr6+)" isCompleted={completed} isLocked={locked} completedAt={completedAt} canUnlockPreparation={canUnlockPreparation} onComplete={() => setShowComplete(true)} onUnlock={() => setShowUnlock(true)} />
          </div>
          {completed && <HexavalentChromiumCalculationSection calculations={calculations} samplePreparations={samplePreparations} canEditCalculations={canEditCalculations} onAdd={() => {
                        setCalculations((items) => [...items, { ...createCalculationHexavalentChromium(items.length), selectedSamplePreparationLabel: samplePreparations[0]?.label ?? null }]);
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="Hexavalent Chromium (as Cr6+)" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
HexavalentChromiumPreparationModule.displayName = "HexavalentChromiumPreparationModule";
export default HexavalentChromiumPreparationModule;
