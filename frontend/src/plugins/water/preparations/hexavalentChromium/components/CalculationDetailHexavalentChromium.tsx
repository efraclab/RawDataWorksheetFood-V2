...
import type { SamplePreparationHexavalentChromium } from "../models/SamplePreparationHexavalentChromium";
import { calculateHexavalentChromium } from "../calculation";
...
const step = (preparation: SamplePreparationHexavalentChromium | null, name: string) =>
  preparation?.steps.find(
    (item) => item.name.trim().toLowerCase() === name.trim().toLowerCase(),
  );

interface Props {
  calculation: CalculationHexavalentChromium;
  samplePreparations: SamplePreparationHexavalentChromium[];
  onUpdate: (calculation: CalculationHexavalentChromium) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailHexavalentChromium({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const update = <K extends keyof CalculationHexavalentChromium>(
    field: K,
    value: CalculationHexavalentChromium[K],
  ...
  const runCalculation = () => {
    ...
    const result = calculateHexavalentChromium(values);
    onUpdate({
      ...calculation,
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        ...
        >
          {calculation.label} · Hexavalent Chromium (as Cr6+)
        </button>
