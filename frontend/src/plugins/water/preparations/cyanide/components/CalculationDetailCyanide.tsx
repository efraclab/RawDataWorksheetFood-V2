...
import { calculateCyanide } from "../calculation";
...
const step = (preparation: SamplePreparationCyanide | null, name: string) =>
  preparation?.steps.find(
    (item) => item.name.trim().toLowerCase() === name.trim().toLowerCase(),
  );

interface Props {
  calculation: CalculationCyanide;
...
export default function CalculationDetailCyanide({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const update = <K extends keyof CalculationCyanide>(
    field: K,
    value: CalculationCyanide[K],
  ...
  const runCalculation = () => {
    ...
    const result = calculateCyanide(values);
    onUpdate({
      ...calculation,
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        ...
        >
          {calculation.label} · Cyanide (as CN)
        </button>
