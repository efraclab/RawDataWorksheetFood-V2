...
import type { SamplePreparationCyanide, SamplePreparationCyanideStep } from "../models/SamplePreparationCyanide";
import type { CalculationCyanide } from "../models/CalculationCyanide";
import { createCalculationCyanide, createSamplePreparationCyanide, restoreCalculationCyanide } from "../factory";
import CyanideCalculationSection from "./CyanideCalculationSection";
...
export interface CyanidePreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationCyanide => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationCyanide>;
  const steps = Array.isArray(source.steps) ? source.steps : createSamplePreparationCyanide(index).steps;
  return {
    ...createSamplePreparationCyanide(index),
    ...source,
    id: typeof source.id === "number" ? source.id : index + 1,
    label: typeof source.label === "string" ? source.label : `Sample Preparation ${index + 1}`,
    steps: steps as SamplePreparationCyanideStep[],
  };
};

const CyanidePreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, CyanidePreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationCyanide[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationCyanide[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationCyanide(items.length)]);
      notify("Cyanide sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationCyanideStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("Cyanide preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("Cyanide preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationCyanide[], preparations: SamplePreparationCyanide[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationCyanide(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "cyanide")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "cyanide")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationCyanide(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">Cyanide (as CN) Analysis</h2>
                <p className="text-sm text-emerald-600">Water Laboratory • Cyanide Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for Cyanide (as CN)"
            ...
            <div className="mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white">
              <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
                <div>
                  ...
                  <div className="text-xs text-emerald-100">Cyanide (as CN) inputs</div>
                </div>
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach Cyanide (as CN) Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="Cyanide (as CN)" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
CyanidePreparationModule.displayName = "CyanidePreparationModule";
export default CyanidePreparationModule;
