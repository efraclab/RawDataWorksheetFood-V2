...
import type { SamplePreparationAlkalinity, SamplePreparationAlkalinityStep } from "../models/SamplePreparationAlkalinity";
import type { CalculationAlkalinity } from "../models/CalculationAlkalinity";
import { createCalculationAlkalinity, createSamplePreparationAlkalinity, restoreCalculationAlkalinity } from "../factory";
import AlkalinityCalculationSection from "./AlkalinityCalculationSection";

type ModuleData = {
  samplePreparations: SamplePreparationAlkalinity[];
  files: PreparationAttachedFile[];
  calculations: CalculationAlkalinity[];
  completed: boolean;
  completedAt: string | null;
};

export interface AlkalinityPreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationAlkalinity => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationAlkalinity>;
  const steps = Array.isArray(source.steps) ? source.steps : createSamplePreparationAlkalinity(index).steps;
  return {
    ...createSamplePreparationAlkalinity(index),
    ...source,
    id: typeof source.id === "number" ? source.id : index + 1,
    label: typeof source.label === "string" ? source.label : `Sample Preparation ${index + 1}`,
    steps: steps as SamplePreparationAlkalinityStep[],
  };
};

const AlkalinityPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, AlkalinityPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationAlkalinity[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationAlkalinity[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationAlkalinity(items.length)]);
      notify("Alkalinity sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationAlkalinityStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("Alkalinity preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("Alkalinity preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationAlkalinity[], preparations: SamplePreparationAlkalinity[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationAlkalinity(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "alkalinity")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "alkalinity")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationAlkalinity(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">Alkalinity (as Alkalinity) Analysis</h2>
                <p className="text-sm text-emerald-600">Water Laboratory • Alkalinity Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for Alkalinity (as Alkalinity)"
            ...
            <div className="mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white">
              <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
                <div>
                  ...
                  <div className="text-xs text-emerald-100">Alkalinity (as Alkalinity) inputs</div>
                </div>
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach Alkalinity (as Alkalinity) Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
            <PreparationCompleteSection preparationName="Alkalinity (as Alkalinity)" isCompleted={completed} isLocked={locked} completedAt={completedAt} canUnlockPreparation={canUnlockPreparation} onComplete={() => setShowComplete(true)} onUnlock={() => setShowUnlock(true)} />
          </div>
          {completed && <AlkalinityCalculationSection calculations={calculations} samplePreparations={samplePreparations} canEditCalculations={canEditCalculations} onAdd={() => {
            setCalculations((items) => [...items, { ...createCalculationAlkalinity(items.length), selectedSamplePreparationLabel: samplePreparations[0]?.label ?? null }]);
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="Alkalinity (as Alkalinity)" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
AlkalinityPreparationModule.displayName = "AlkalinityPreparationModule";
export default AlkalinityPreparationModule;
