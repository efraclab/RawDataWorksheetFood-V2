...
import type { SamplePreparationTotalPhosphorus, SamplePreparationTotalPhosphorusStep } from "../models/SamplePreparationTotalPhosphorus";
import type { CalculationTotalPhosphorus } from "../models/CalculationTotalPhosphorus";
import { createCalculationTotalPhosphorus, createSamplePreparationTotalPhosphorus, restoreCalculationTotalPhosphorus } from "../factory";
import TotalPhosphorusCalculationSection from "./TotalPhosphorusCalculationSection";

type ModuleData = {
  samplePreparations: SamplePreparationTotalPhosphorus[];
  files: PreparationAttachedFile[];
  calculations: CalculationTotalPhosphorus[];
  completed: boolean;
  completedAt: string | null;
};

export interface TotalPhosphorusPreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationTotalPhosphorus => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationTotalPhosphorus>;
  const steps = Array.isArray(source.steps) ? source.steps : createSamplePreparationTotalPhosphorus(index).steps;
  return {
    ...createSamplePreparationTotalPhosphorus(index),
    ...source,
    id: typeof source.id === "number" ? source.id : index + 1,
    label: typeof source.label === "string" ? source.label : `Sample Preparation ${index + 1}`,
    steps: steps as SamplePreparationTotalPhosphorusStep[],
  };
};

const TotalPhosphorusPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, TotalPhosphorusPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationTotalPhosphorus[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationTotalPhosphorus[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationTotalPhosphorus(items.length)]);
      notify("TotalPhosphorus sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationTotalPhosphorusStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("TotalPhosphorus preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("TotalPhosphorus preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationTotalPhosphorus[], preparations: SamplePreparationTotalPhosphorus[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationTotalPhosphorus(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "totalphosphorus")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "totalphosphorus")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationTotalPhosphorus(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">Total Phosphorus Analysis</h2>
                <p className="text-sm text-emerald-600">Water Laboratory • TotalPhosphorus Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for Total Phosphorus"
            ...
            <div className="mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white">
              <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
                <div>
                  ...
                  <div className="text-xs text-emerald-100">Total Phosphorus inputs</div>
                </div>
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach Total Phosphorus Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
            <PreparationCompleteSection preparationName="Total Phosphorus" isCompleted={completed} isLocked={locked} completedAt={completedAt} canUnlockPreparation={canUnlockPreparation} onComplete={() => setShowComplete(true)} onUnlock={() => setShowUnlock(true)} />
          </div>
          {completed && <TotalPhosphorusCalculationSection calculations={calculations} samplePreparations={samplePreparations} canEditCalculations={canEditCalculations} onAdd={() => {
                        setCalculations((items) => [...items, { ...createCalculationTotalPhosphorus(items.length), selectedSamplePreparationLabel: samplePreparations[0]?.label ?? null }]);
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="Total Phosphorus" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
TotalPhosphorusPreparationModule.displayName = "TotalPhosphorusPreparationModule";
export default TotalPhosphorusPreparationModule;
