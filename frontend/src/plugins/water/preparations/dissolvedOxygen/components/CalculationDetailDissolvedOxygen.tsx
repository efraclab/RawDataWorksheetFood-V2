...
import type { SamplePreparationDissolvedOxygen } from "../models/SamplePreparationDissolvedOxygen";
import { calculateDissolvedOxygen } from "../calculation";
...
const step = (preparation: SamplePreparationDissolvedOxygen | null, name: string) =>
  preparation?.steps.find(
    (item) => item.name.trim().toLowerCase() === name.trim().toLowerCase(),
  );

interface Props {
  calculation: CalculationDissolvedOxygen;
  samplePreparations: SamplePreparationDissolvedOxygen[];
  onUpdate: (calculation: CalculationDissolvedOxygen) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailDissolvedOxygen({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const values = useMemo(() => {
    const volume = step(selected, "Volume of Na2S2O3");
    const strength = step(selected, "Strength of Na2S2O3");

    return { volume: volume?.value1 ?? "", strength: strength?.value1 ?? "" };
  }, [selected]);

  const requiredKeys = ["volume", "strength"] as const;
  ...
  const update = <K extends keyof CalculationDissolvedOxygen>(
    field: K,
    value: CalculationDissolvedOxygen[K],
  ...
  const runCalculation = () => {
    ...
    const result = calculateDissolvedOxygen(values);
    onUpdate({
      ...calculation,
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        ...
        >
          {calculation.label} · Dissolved Oxygen (as O2)
        </button>
          ...
          {selected && (
            <>
              <div className="rounded-lg border border-emerald-200 bg-slate-50 p-4 text-center font-mono text-xs">
                Dissolved oxygen (mg/L) = (Volume of Na2S2O3 × Strength of Na2S2O3) / 0.025
              </div>
