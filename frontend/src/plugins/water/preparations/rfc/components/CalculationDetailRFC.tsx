...
import type { SamplePreparationRFC } from "../models/SamplePreparationRFC";
import { calculateRFC } from "../calculation";
...
const step = (preparation: SamplePreparationRFC | null, name: string) =>
  preparation?.steps.find(
    (item) => item.name.trim().toLowerCase() === name.trim().toLowerCase(),
  );

interface Props {
  calculation: CalculationRFC;
  samplePreparations: SamplePreparationRFC[];
  onUpdate: (calculation: CalculationRFC) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailRFC({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const update = <K extends keyof CalculationRFC>(
    field: K,
    value: CalculationRFC[K],
  ...
  const runCalculation = () => {
    ...
    const result = calculateRFC(values);
    onUpdate({
      ...calculation,
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        ...
        >
          {calculation.label} · Residual Free Chlorine (as Cl2)
        </button>
