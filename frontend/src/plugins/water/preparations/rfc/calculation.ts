...
export interface RFCCalculationInput {
  abs: unknown;
  df: unknown;
  m: unknown;
  c: unknown;
...
export interface RFCCalculationResult {
  success: boolean;
  result: number | null;
  error?: string;
}
 ...
 */
export function calculateRFC(
  input: RFCCalculationInput,
): RFCCalculationResult {
  ...
  if ([abs, df, m, c].some((value) => value === null)) {
    return { success: false, result: null, error: "All RFC values are required." };
  }
  ...
  return Number.isFinite(result)
    ...
    : { success: false, result: null, error: "Unable to calculate RFC concentration." };
}
