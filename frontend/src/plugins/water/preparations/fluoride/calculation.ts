...
export interface FluorideCalculationInput {
  abs: unknown;
  df: unknown;
  m: unknown;
  c: unknown;
...
export interface FluorideCalculationResult {
  success: boolean;
  result: number | null;
  error?: string;
}
 ...
 */
export function calculateFluoride(
  input: FluorideCalculationInput,
): FluorideCalculationResult {
  ...
  if ([abs, df, m, c].some((value) => value === null)) {
    return { success: false, result: null, error: "All Fluoride values are required." };
  }
  ...
  return Number.isFinite(result)
    ...
    : { success: false, result: null, error: "Unable to calculate Fluoride concentration." };
}
