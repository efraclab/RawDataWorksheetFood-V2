...
import type { SamplePreparationTotalHardness, SamplePreparationTotalHardnessStep } from "../models/SamplePreparationTotalHardness";
import type { CalculationTotalHardness } from "../models/CalculationTotalHardness";
import { createCalculationTotalHardness, createSamplePreparationTotalHardness, restoreCalculationTotalHardness } from "../factory";
import TotalHardnessCalculationSection from "./TotalHardnessCalculationSection";

type ModuleData = {
  samplePreparations: SamplePreparationTotalHardness[];
  files: PreparationAttachedFile[];
  calculations: CalculationTotalHardness[];
  completed: boolean;
  completedAt: string | null;
};

export interface TotalHardnessPreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationTotalHardness => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationTotalHardness>;
  const steps = Array.isArray(source.steps) ? source.steps : createSamplePreparationTotalHardness(index).steps;
  return {
    ...createSamplePreparationTotalHardness(index),
    ...source,
    id: typeof source.id === "number" ? source.id : index + 1,
    label: typeof source.label === "string" ? source.label : `Sample Preparation ${index + 1}`,
    steps: steps as SamplePreparationTotalHardnessStep[],
  };
};

const TotalHardnessPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, TotalHardnessPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationTotalHardness[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationTotalHardness[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationTotalHardness(items.length)]);
      notify("TotalHardness sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationTotalHardnessStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("TotalHardness preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("TotalHardness preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationTotalHardness[], preparations: SamplePreparationTotalHardness[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationTotalHardness(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "totalhardness")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "totalhardness")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationTotalHardness(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">Total Hardness (as CaCO3) Analysis</h2>
                <p className="text-sm text-emerald-600">Water Laboratory • TotalHardness Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for Total Hardness (as CaCO3)"
            ...
            <div className="mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white">
              <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
                <div>
                  ...
                  <div className="text-xs text-emerald-100">Total Hardness (as CaCO3) inputs</div>
                </div>
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach Total Hardness (as CaCO3) Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
            <PreparationCompleteSection preparationName="Total Hardness (as CaCO3)" isCompleted={completed} isLocked={locked} completedAt={completedAt} canUnlockPreparation={canUnlockPreparation} onComplete={() => setShowComplete(true)} onUnlock={() => setShowUnlock(true)} />
          </div>
          {completed && <TotalHardnessCalculationSection calculations={calculations} samplePreparations={samplePreparations} canEditCalculations={canEditCalculations} onAdd={() => {
                        setCalculations((items) => [...items, { ...createCalculationTotalHardness(items.length), selectedSamplePreparationLabel: samplePreparations[0]?.label ?? null }]);
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="Total Hardness (as CaCO3)" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
TotalHardnessPreparationModule.displayName = "TotalHardnessPreparationModule";
export default TotalHardnessPreparationModule;
