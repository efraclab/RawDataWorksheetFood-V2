...
export interface MBASCalculationInput {
  abs: unknown;
  df: unknown;
  m: unknown;
  c: unknown;
...
export interface MBASCalculationResult {
  success: boolean;
  result: number | null;
  error?: string;
}
...
export function calculateMBAS(
  ...
  if ([abs, df, m, c].some((value) => value === null)) {
    return { success: false, result: null, error: "All MBAS values are required." };
  }
  ...
  const result = ((abs! + c!) * df!) / (m! * 1000);
  return Number.isFinite(result)
    ? { success: true, result: Math.trunc(result * 1000) / 1000 }
    : { success: false, result: null, error: "Unable to calculate MBAS concentration." };
}
