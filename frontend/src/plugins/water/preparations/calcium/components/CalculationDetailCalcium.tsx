...
import type { SamplePreparationCalcium } from "../models/SamplePreparationCalcium";
...
const step = (preparation: SamplePreparationCalcium | null, name: string) =>
  preparation?.steps.find((item) => item.name.trim().toLowerCase() === name.trim().toLowerCase());

interface Props {
  calculation: CalculationCalcium;
  samplePreparations: SamplePreparationCalcium[];
  onUpdate: (calculation: CalculationCalcium) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailCalcium({ calculation, samplePreparations, onUpdate, onRemove, isLocked }: Props) {
  ...
  const update = <K extends keyof CalculationCalcium>(field: K, value: CalculationCalcium[K]) => onUpdate({ ...calculation, [field]: value });
  const runCalculation = () => {
    if (!selected || errors.length > 0) return;
    const result = calculateCalcium(values);
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        <button type="button" onClick={() => setExpanded((value) => !value)} className="font-semibold focus:outline-none focus:ring-0 focus:ring-0">{calculation.label} · Calcium</button>
          ...
          {selected && (
            <>
              <div className="space-y-2 rounded-lg border border-emerald-200 bg-slate-50 p-4 text-center font-mono text-xs leading-6">
                Calcium Hardness (CaCO3) = (Volume of EDTA × Strength × 100 × 1000 × DF) / Volume of sample<br />Calcium (Ca) = Calcium Hardness (CaCO3) × 0.4008
              </div>
              {errors.length > 0 && (
