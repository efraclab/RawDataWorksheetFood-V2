...
import type { SamplePreparationChloramines, SamplePreparationChloraminesStep } from "../models/SamplePreparationChloramines";
import type { CalculationChloramines } from "../models/CalculationChloramines";
import { createCalculationChloramines, createSamplePreparationChloramines, restoreCalculationChloramines } from "../factory";
import ChloraminesCalculationSection from "./ChloraminesCalculationSection";

type ModuleData = {
  samplePreparations: SamplePreparationChloramines[];
  files: PreparationAttachedFile[];
  calculations: CalculationChloramines[];
  completed: boolean;
  completedAt: string | null;
};

export interface ChloraminesPreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationChloramines => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationChloramines>;
  const steps = Array.isArray(source.steps) ? source.steps : createSamplePreparationChloramines(index).steps;
  return {
    ...createSamplePreparationChloramines(index),
    ...source,
    id: typeof source.id === "number" ? source.id : index + 1,
    label: typeof source.label === "string" ? source.label : `Sample Preparation ${index + 1}`,
    steps: steps as SamplePreparationChloraminesStep[],
  };
};

const ChloraminesPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, ChloraminesPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationChloramines[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationChloramines[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationChloramines(items.length)]);
      notify("Chloramines sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationChloraminesStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("Chloramines preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("Chloramines preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationChloramines[], preparations: SamplePreparationChloramines[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationChloramines(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "chloramines")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "chloramines")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationChloramines(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">Chloramine (Cl2) Analysis</h2>
                <p className="text-sm text-emerald-600">Water Laboratory • Chloramines Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for Chloramine (Cl2)"
            ...
            <div className="mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white">
              <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
                <div>
                  ...
                  <div className="text-xs text-emerald-100">Chloramine (Cl2) inputs</div>
                </div>
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach Chloramine (Cl2) Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
            <PreparationCompleteSection preparationName="Chloramine (Cl2)" isCompleted={completed} isLocked={locked} completedAt={completedAt} canUnlockPreparation={canUnlockPreparation} onComplete={() => setShowComplete(true)} onUnlock={() => setShowUnlock(true)} />
          </div>
          {completed && <ChloraminesCalculationSection calculations={calculations} samplePreparations={samplePreparations} canEditCalculations={canEditCalculations} onAdd={() => {
                        setCalculations((items) => [...items, { ...createCalculationChloramines(items.length), selectedSamplePreparationLabel: samplePreparations[0]?.label ?? null }]);
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="Chloramine (Cl2)" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
ChloraminesPreparationModule.displayName = "ChloraminesPreparationModule";
export default ChloraminesPreparationModule;
