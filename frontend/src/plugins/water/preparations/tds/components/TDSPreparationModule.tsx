...
import { createCalculationTDS, createSamplePreparationTDS, restoreCalculationTDS } from "../factory";
import TDSCalculationSection from "./TDSCalculationSection";

type ModuleData = {
  samplePreparations: SamplePreparationTDS[];
  files: PreparationAttachedFile[];
  calculations: CalculationTDS[];
  completed: boolean;
  completedAt: string | null;
};

export interface TDSPreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationTDS => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationTDS>;
  const steps = Array.isArray(source.steps) ? source.steps : createSamplePreparationTDS(index).steps;
  return {
    ...createSamplePreparationTDS(index),
    ...source,
    id: typeof source.id === "number" ? source.id : index + 1,
    label: typeof source.label === "string" ? source.label : `Sample Preparation ${index + 1}`,
    steps: steps as SamplePreparationTDSStep[],
  };
};

const TDSPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, TDSPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationTDS[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationTDS[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationTDS(items.length)]);
      notify("TDS sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationTDSStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("TDS preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("TDS preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationTDS[], preparations: SamplePreparationTDS[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationTDS(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "tds")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "tds")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationTDS(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">Total Dissolved Solids (TDS) Analysis</h2>
                <p className="text-sm text-emerald-600">Water Laboratory • TDS Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for Total Dissolved Solids (TDS)"
            ...
            <div className="mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white">
              <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
                <div>
                  ...
                  <div className="text-xs text-emerald-100">Total Dissolved Solids (TDS) inputs</div>
                </div>
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach Total Dissolved Solids (TDS) Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="Total Dissolved Solids (TDS)" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
TDSPreparationModule.displayName = "TDSPreparationModule";
export default TDSPreparationModule;
