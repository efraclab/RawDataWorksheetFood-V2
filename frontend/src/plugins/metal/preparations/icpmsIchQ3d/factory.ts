...
export function createCalculationIcpmsIchQ3d(index: number): CalculationIcpmsIchQ3d {
  return {
    id: createId(index),
...
export function createSamplePreparationIcpmsIchQ3d(
  index: number,
): SamplePreparationIcpmsIchQ3d {
  return {
    id: createId(index),
...
export function restoreCalculationIcpmsIchQ3d(
  value: unknown,
  index = 0,
): CalculationIcpmsIchQ3d {
  const source =
    value && typeof value === "object"
      ? (value as Partial<CalculationIcpmsIchQ3d> & {
          sw?: number | string | null;
        })
      : {};

  const created = createCalculationIcpmsIchQ3d(index);
 ...
 */
export function restoreSamplePreparationIcpmsIchQ3d(
  value: unknown,
  index = 0,
): SamplePreparationIcpmsIchQ3d {
  const created = createSamplePreparationIcpmsIchQ3d(index);

  if (!value || typeof value !== "object") return created;

  const source = value as Partial<SamplePreparationIcpmsIchQ3d>;
  ...
  const oldByName = new Map(
    rawSteps
      .filter((step): step is SamplePreparationIcpmsIchQ3dStep =>
  ...
  const mapOldStep = (
    defaultStep: SamplePreparationIcpmsIchQ3dStep,
    oldStep: SamplePreparationIcpmsIchQ3dStep | undefined,
  ): SamplePreparationIcpmsIchQ3dStep => ({
    ...defaultStep,
