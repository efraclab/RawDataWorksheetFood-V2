...
import type {
  IcpmsIchQ3dModuleData,
  IcpmsIchQ3dModuleDraft,
} from "../models/index";
import {
...
import IcpmsIchQ3dCalculationSection from "./IcpmsIchQ3dCalculationSection";

export interface IcpmsIchQ3dPreparationModuleProps {
...
const parseSteps = (value: unknown): SamplePreparationIcpmsIchQ3dStep[] => {
  if (Array.isArray(value)) return value as SamplePreparationIcpmsIchQ3dStep[];
  if (typeof value !== "string") return [];

  try {
    const parsed = JSON.parse(value);
    return Array.isArray(parsed)
      ? (parsed as SamplePreparationIcpmsIchQ3dStep[])
      : [];
  } catch {
    return [];
  }
 ...
 */
const IcpmsIchQ3dStep = ({
  step,
  stepNumber,
  locked,
  onChange,
}: {
  step: SamplePreparationIcpmsIchQ3dStep;
  stepNumber: number;
  locked: boolean;
  onChange: (field: keyof SamplePreparationIcpmsIchQ3dStep, value: string) => void;
}) => {
...
const IcpmsIchQ3dSamplePreparationCard = ({
  preparation,
  disabled,
  onRemove,
  onChange,
}: {
  preparation: SamplePreparationIcpmsIchQ3d;
  disabled: boolean;
  onRemove: () => void;
  onChange: (
    stepIndex: number,
    field: keyof SamplePreparationIcpmsIchQ3dStep,
    value: string,
  ) => void;
}) => {
  return (
    <div className="group relative z-20">
      ...
      <div className="relative mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white/95">
            ...
            <div className="flex flex-1 items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-100">
                  Sample Preparation for ICP-MS (ICH-Q3D)
                </p>
              </div>
            </div>
              ...
              <button
                ...
                onClick={() => {
                  const element = document.getElementById(
                    `icpmsIchQ3d-prep-${preparation.id}`,
                  );
        ...
        <div
          id={`icpmsIchQ3d-prep-${preparation.id}`}
...
const IcpmsIchQ3dPreparationModule = forwardRef<
  PreparationModuleHandle<IcpmsIchQ3dModuleDraft, unknown>,
  IcpmsIchQ3dPreparationModuleProps
>((props, ref) => {
  const {
    parameterId,
    parameterName,
  ...
  const [calculations, setCalculations] = useState<CalculationIcpmsIchQ3d[]>([]);
  ...
  const addPreparation = () => {
    ...
    setSamplePreparations((current) => [
      ...current,
      createSamplePreparationIcpmsIchQ3d(current.length),
    ]);
  ...
  const updateCalculation = (updated: CalculationIcpmsIchQ3d) => {
    setCalculations((current) =>
      current.map((calculation) =>
  ...
  useImperativeHandle(
    ref,
    () => ({
      getDraft: (): IcpmsIchQ3dModuleDraft => ({
        samplePreparations,
        files,
        calculations,
        completed,
        ...
        setCalculations(
          Array.isArray(value.calculations)
            ? value.calculations.map((item, index) =>
                restoreCalculationIcpmsIchQ3d(item, index),
              )
            : [],
        );
        ...
        const preparations = Array.isArray(parameter.preparations)
          ? parameter.preparations
              ...
              .map((item, index) => {
                ...
                return restoreSamplePreparationIcpmsIchQ3d(
                  {
                    ...value,
        ...
        const worksheetCalculations = Array.isArray(parameter.calculations)
          ? parameter.calculations
              ...
              .map((item, index) => {
                ...
                return restoreCalculationIcpmsIchQ3d(data ?? value, index);
              })
          : [];
  ...
  return (
    <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
      ...
      <SamplePreparationSection<SamplePreparationIcpmsIchQ3d>
        title="Sample Preparations for ICP-MS (ICH-Q3D)"
        ...
        renderPreparation={(preparation, index) => (
          <IcpmsIchQ3dSamplePreparationCard
        ...
        <>
          ...
          <div className="mx-6">
            <PreparationCompleteSection
              preparationName="ICP-MS (ICH-Q3D)"
          ...
          {completed && (
            <IcpmsIchQ3dCalculationSection
              ...
              onAdd={() =>
                ...
                setCalculations((current) => [
                  ...current,
                  createCalculationIcpmsIchQ3d(current.length),
                ])
              }
              onRemove={(id) => {
...
IcpmsIchQ3dPreparationModule.displayName = "IcpmsIchQ3dPreparationModule";

export default IcpmsIchQ3dPreparationModule;
