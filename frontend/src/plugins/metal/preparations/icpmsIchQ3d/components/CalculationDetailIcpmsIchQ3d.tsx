...
export interface CalculationDetailIcpmsIchQ3dProps {
...
const findStep = (
  preparation: SamplePreparationIcpmsIchQ3d | null,
  name: string,
) => preparation?.steps.find((step) => step.name === name);

/**
 * ICP-MS calculation UI follows the LOD calculation structure.
 *
 * IMPORTANT:
 * Instrument Concentration (Sample) and Instrument Concentration (Blank)
 * are already entered in the ICP-MS Sample Preparation. Therefore they are
 ...
 */
const CalculationDetailIcpmsIchQ3d: React.FC<CalculationDetailIcpmsIchQ3dProps> = ({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const update = <K extends keyof CalculationIcpmsIchQ3d>(
    field: K,
    value: CalculationIcpmsIchQ3d[K],
  ) => {
    /*
  ...
  const performCalculation = () => {
    ...
    const result = calculateIcpmsIchQ3d({
          ...
          >
            ...
            <span className="min-w-0">
              ...
              <span className="block text-xs text-emerald-100">
                Calculation for ICP-MS (ICH-Q3D)
              </span>
            </span>
          </button>
...
export default CalculationDetailIcpmsIchQ3d;
