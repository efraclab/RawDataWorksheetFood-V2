...
import CalculationDetailAasWater from "./CalculationDetailAasWater";

interface Props {
  calculations: CalculationAasWater[];
...
/**
 * AAS (Water) calculation section intentionally mirrors the LOD calculation
 * section. The old Excel-style calculation table/formula header is not
 * rendered here.
 */
const AasWaterCalculationSection: React.FC<Props> = ({
  calculations,
  samplePreparations,
  canEditCalculations,
  onAdd,
        ...
        {calculations.length === 0 ? (
          <div className="rounded-xl border-2 border-dashed border-emerald-300 bg-gradient-to-br from-emerald-50 via-white to-emerald-50 py-12 text-center shadow-inner">
            ...
            <p className="mb-1 mt-3 text-base font-semibold text-emerald-800">
              No AAS (Water) calculations added yet
            </p>
...
export default AasWaterCalculationSection;
