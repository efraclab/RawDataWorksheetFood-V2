...
import type { SamplePreparationSfgc } from "../models/SamplePreparationSfgc";
...
const step = (preparation: SamplePreparationSfgc | null, name: string) =>
  preparation?.steps.find((item) => item.name.trim().toLowerCase() === name.trim().toLowerCase());

interface Props {
  calculation: CalculationSfgc;
  samplePreparations: SamplePreparationSfgc[];
  onUpdate: (calculation: CalculationSfgc) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailSfgc({ calculation, samplePreparations, onUpdate, onRemove, isLocked }: Props) {
  ...
  const values = useMemo(() => {
    ...
    const weight = step(selected, "Sample Weight (SW)");
    return {
  ...
  const requiredKeys = ["sample", "blank", "v1Volume", "v2Factor", "v2Volume", "v3Factor", "v3Volume", "sampleWeight"] as const;
  const errors = requiredKeys.filter((key) => text(values[key]).trim() === "" || numberOrNull(values[key]) === null);
  const update = <K extends keyof CalculationSfgc>(field: K, value: CalculationSfgc[K]) =>
    onUpdate({ ...calculation, [field]: value });

  const runCalculation = () => {
    if (isLocked || !selected || errors.length > 0) return;
    const result = calculateSfgc(values);
    onUpdate({
      ...calculation,
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        <button type="button" onClick={() => setExpanded((value) => !value)} className="font-semibold">{calculation.label} · SFGC</button>
          ...
          {selected && (
            <>
              <div className="rounded-lg border border-emerald-200 bg-slate-50 p-4 text-center font-mono text-xs">
                ((Sample − Blank) × V2(volume) × V3(volume) × 1000 × 100) / (SW × V2(factor) × V3(factor) × 10,000)
              </div>
              ...
              {resultText && (
                <div className="overflow-hidden rounded-lg border-2 border-emerald-300">
                  <div className="bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-2 font-bold text-white">SFGC Result</div>
