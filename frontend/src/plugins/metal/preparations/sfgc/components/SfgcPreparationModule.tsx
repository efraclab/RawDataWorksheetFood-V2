...
import SfgcCalculationSection from "./SfgcCalculationSection";

export interface SfgcPreparationModuleProps {
...
const parseSteps = (value: unknown): SamplePreparationSfgcStep[] => {
  if (Array.isArray(value)) return value as SamplePreparationSfgcStep[];
  if (typeof value !== "string") return [];

  try {
    const parsed = JSON.parse(value);
    return Array.isArray(parsed)
      ? (parsed as SamplePreparationSfgcStep[])
      : [];
  } catch {
    return [];
  }
...
const getUnitOptions = (name: string): string[] => {
  ...
  if (name === "Sample Weight (SW)") return ["mg", "g", "kg"];
...
const getDefaultUnit = (name: string): string => {
  ...
  if (name === "Sample Weight (SW)") return "mg";
...
const SfgcStep = ({
  ...
  step: SamplePreparationSfgcStep;
  stepNumber: number;
  locked: boolean;
  onChange: (field: keyof SamplePreparationSfgcStep, value: string) => void;
}) => {
  ...
  const isV2 = step.name === "Dilution Factor 2 (V2)";
        ...
        <div className="min-w-0 flex-1">
          ...
          {(isV2 || isV3 || isV4) && (
            <p className="mt-2 text-[11px] text-slate-500">
              Excel input: <span className="font-semibold">factor × volume</span> — both values are used in the SFGC calculation.
            </p>
          )}
          {isLabelClaim && (
            <p className="mt-2 text-[11px] text-slate-500">
              Excel input: <span className="font-semibold">100 / claim value</span> — both values are used in the SFGC calculation.
            </p>
          )}
        </div>
      </div>
...
const SfgcSamplePreparationCard = ({
  preparation,
  disabled,
  onRemove,
  onChange,
}: {
  preparation: SamplePreparationSfgc;
  disabled: boolean;
  onRemove: () => void;
  onChange: (
    stepIndex: number,
    field: keyof SamplePreparationSfgcStep,
    value: string,
  ) => void;
}) => {
  return (
...
const SfgcPreparationModule = forwardRef<
  PreparationModuleHandle<SfgcModuleDraft, unknown>,
  SfgcPreparationModuleProps
>((props, ref) => {
  const {
    parameterId,
    parameterName,
  ...
  const [calculations, setCalculations] = useState<CalculationSfgc[]>([]);
  ...
  const addPreparation = () => {
    ...
    setSamplePreparations((current) => [
      ...current,
      createSamplePreparationSfgc(current.length),
    ]);
    notify("SFGC sample preparation added.");
  };
  ...
  const complete = () => {
    ...
    notify("SFGC preparation marked as complete!");
  };
  ...
  const unlock = async () => {
    ...
    try {
      ...
      setShowUnlock(false);
      notify("SFGC preparation unlocked successfully.");
    } finally {
      setUnlocking(false);
    }
  };

  const updateCalculation = (updated: CalculationSfgc) => {
    setCalculations((current) =>
      current.map((calculation) =>
        ...
        setCalculations(
          Array.isArray(value.calculations)
            ? value.calculations.map((item, index) =>
                restoreCalculationSfgc(item, index),
              )
            : [],
        );
        ...
        const preparations = Array.isArray(parameter.preparations)
          ? parameter.preparations
              ...
              .map((item, index) => {
                ...
                return restoreSamplePreparationSfgc(
                  {
                    ...value,
        ...
        const worksheetCalculations = Array.isArray(parameter.calculations)
          ? parameter.calculations
              ...
              .map((item, index) => {
                ...
                return restoreCalculationSfgc(data ?? value, index);
              })
          : [];
                ...
                return {
                  id:
                    typeof value.id === "number"
                      ...
                      : `${Date.now()}-${index}`,
                  name: String(
                    value.fileName ?? value.name ?? `SFGC-${index + 1}.pdf`,
                  ),
  ...
  return (
    <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-600">
                Metal Laboratory • SFGC • Sample &amp; Calculations
              </p>
            </div>
          </div>
      ...
      <SamplePreparationSection<SamplePreparationSfgc>
        title="Sample Preparations for SFGC"
        ...
        <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection
              files={files}
              isLocked={preparationDataLocked}
              title="Attach SFGC Preparation Files (PDF)"
              maxFiles={10}
          ...
          {completed && (
            <SfgcCalculationSection
              ...
              onAdd={() =>
                ...
                setCalculations((current) => [
                  ...current,
                  createCalculationSfgc(current.length),
                ])
              }
              onRemove={(id) => {
...
SfgcPreparationModule.displayName = "SfgcPreparationModule";

export default SfgcPreparationModule;
