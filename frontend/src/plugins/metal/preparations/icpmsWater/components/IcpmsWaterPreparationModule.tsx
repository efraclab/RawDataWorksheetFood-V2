...
import type {
  IcpmsWaterModuleData,
  IcpmsWaterModuleDraft,
} from "../models/index";
import {
  createCalculationIcpmsWater,
  createSamplePreparationIcpmsWater,
  restoreCalculationIcpmsWater,
  restoreSamplePreparationIcpmsWater,
} from "../factory";
import IcpmsWaterCalculationSection from "./IcpmsWaterCalculationSection";

export interface IcpmsWaterPreparationModuleProps {
...
const parseSteps = (value: unknown): SamplePreparationIcpmsWaterStep[] => {
  if (Array.isArray(value)) return value as SamplePreparationIcpmsWaterStep[];
  if (typeof value !== "string") return [];

  try {
    const parsed = JSON.parse(value);
    return Array.isArray(parsed)
      ? (parsed as SamplePreparationIcpmsWaterStep[])
      : [];
  } catch {
    return [];
  }
...
/**
 * ICP-MS (Water) preparation deliberately uses the same visual pattern as LOD.
 *
 * Layout:
 *   1 — full row
 *   2 — full row
...
const IcpmsStep = ({
  ...
  step: SamplePreparationIcpmsWaterStep;
  stepNumber: number;
  locked: boolean;
  onChange: (field: keyof SamplePreparationIcpmsWaterStep, value: string) => void;
}) => {
...
const IcpmsSamplePreparationCard = ({
  ...
  preparation: SamplePreparationIcpmsWater;
  disabled: boolean;
  onRemove: () => void;
  onChange: (
    stepIndex: number,
    field: keyof SamplePreparationIcpmsWaterStep,
    value: string,
  ) => void;
}) => {
  return (
    <div className="group relative z-20">
      ...
      <div className="relative mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white/95">
            ...
            <div className="flex flex-1 items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-100">
                  Sample Preparation for ICP-MS (Water)
                </p>
              </div>
            </div>
              ...
              <button
                ...
                onClick={() => {
                  const element = document.getElementById(
                    `icpmsWater-prep-${preparation.id}`,
                  );
        ...
        <div
          id={`icpmsWater-prep-${preparation.id}`}
...
const IcpmsWaterPreparationModule = forwardRef<
  PreparationModuleHandle<IcpmsWaterModuleDraft, unknown>,
  IcpmsWaterPreparationModuleProps
>((props, ref) => {
  const {
    parameterId,
    parameterName,
  ...
  const [calculations, setCalculations] = useState<CalculationIcpmsWater[]>([]);
  ...
  const addPreparation = () => {
    ...
    setSamplePreparations((current) => [
      ...current,
      createSamplePreparationIcpmsWater(current.length),
    ]);
    notify("ICP-MS (Water) sample preparation added.");
  };
  ...
  const complete = () => {
    ...
    notify("ICP-MS (Water) preparation marked as complete!");
  };
  ...
  const unlock = async () => {
    ...
    try {
      ...
      setShowUnlock(false);
      notify("ICP-MS (Water) preparation unlocked successfully.");
    } finally {
      setUnlocking(false);
    }
  };

  const updateCalculation = (updated: CalculationIcpmsWater) => {
    setCalculations((current) =>
      current.map((calculation) =>
  ...
  useImperativeHandle(
    ref,
    () => ({
      getDraft: (): IcpmsWaterModuleDraft => ({
        samplePreparations,
        files,
        calculations,
        completed,
        ...
        setCalculations(
          Array.isArray(value.calculations)
            ? value.calculations.map((item, index) =>
                restoreCalculationIcpmsWater(item, index),
              )
            : [],
        );
        ...
        const preparations = Array.isArray(parameter.preparations)
          ? parameter.preparations
              ...
              .map((item, index) => {
                ...
                return restoreSamplePreparationIcpmsWater(
                  {
                    ...value,
        ...
        const worksheetCalculations = Array.isArray(parameter.calculations)
          ? parameter.calculations
              ...
              .map((item, index) => {
                ...
                return restoreCalculationIcpmsWater(data ?? value, index);
              })
          : [];
                ...
                return {
                  id:
                    typeof value.id === "number"
                      ...
                      : `${Date.now()}-${index}`,
                  name: String(
                    value.fileName ?? value.name ?? `ICP-MS-Water-${index + 1}.pdf`,
                  ),
  ...
  return (
    <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
      ...
      <SamplePreparationSection<SamplePreparationIcpmsWater>
        title="Sample Preparations for ICP-MS (Water)"
        ...
        <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection
              files={files}
              isLocked={preparationDataLocked}
              title="Attach ICP-MS (Water) Preparation Files (PDF)"
              maxFiles={10}
          ...
          {completed && (
            <IcpmsWaterCalculationSection
              ...
              onAdd={() =>
                ...
                setCalculations((current) => [
                  ...current,
                  createCalculationIcpmsWater(current.length),
                ])
              }
              onRemove={(id) => {
...
IcpmsWaterPreparationModule.displayName = "IcpmsWaterPreparationModule";

export default IcpmsWaterPreparationModule;
