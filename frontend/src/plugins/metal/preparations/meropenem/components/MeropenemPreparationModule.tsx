...
import MeropenemCalculationSection from "./MeropenemCalculationSection";

export interface MeropenemPreparationModuleProps {
...
const parseSteps = (value: unknown): SamplePreparationMeropenemStep[] => {
  if (Array.isArray(value)) return value as SamplePreparationMeropenemStep[];
  if (typeof value !== "string") return [];

  try {
    const parsed = JSON.parse(value);
    return Array.isArray(parsed)
      ? (parsed as SamplePreparationMeropenemStep[])
      : [];
  } catch {
    return [];
  }
...
const getUnitOptions = (name: string): string[] => {
  ...
  if (name === "Makeup Volume (V1)") return ["ml", "L", "µl"];
  return [];
};
...
const getDefaultUnit = (name: string): string => {
  ...
  if (name === "Makeup Volume (V1)") return "ml";
  if (name === "Molecular Weight" || name === "Label Claim") return "-";
  return "";
};

const twoValueStep = (name: string) =>
  name === "Makeup Volume (V1)" ||
...
const MeropenemStep = ({
  ...
  step: SamplePreparationMeropenemStep;
  stepNumber: number;
  locked: boolean;
  onChange: (field: keyof SamplePreparationMeropenemStep, value: string) => void;
}) => {
  ...
  const isV2 = step.name === "Makeup Volume (V1)";
        ...
        <div className="min-w-0 flex-1">
          ...
          {(isV2 || isV3 || isV4) && (
            <p className="mt-2 text-[11px] text-slate-500">
              Excel input: <span className="font-semibold">factor × volume</span> — both values are used in the MEROPENEM calculation.
            </p>
          )}
          {isLabelClaim && (
            <p className="mt-2 text-[11px] text-slate-500">
              Excel input: <span className="font-semibold">100 / claim value</span> — both values are used in the MEROPENEM calculation.
            </p>
          )}
        </div>
      </div>
...
const MeropenemSamplePreparationCard = ({
  preparation,
  disabled,
  onRemove,
  onChange,
}: {
  preparation: SamplePreparationMeropenem;
  disabled: boolean;
  onRemove: () => void;
  onChange: (
    stepIndex: number,
    field: keyof SamplePreparationMeropenemStep,
    value: string,
  ) => void;
}) => {
  return (
    <div className="group relative z-20">
      ...
      <div className="relative mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white/95">
            ...
            <div className="flex flex-1 items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-100">
                  Sample Preparation for MEROPENEM
                </p>
              </div>
            </div>
        ...
        <div
          id={`meropenem-prep-${preparation.id}`}
...
const MeropenemPreparationModule = forwardRef<
  PreparationModuleHandle<MeropenemModuleDraft, unknown>,
  MeropenemPreparationModuleProps
>((props, ref) => {
  const {
    parameterId,
    parameterName,
  ...
  const [calculations, setCalculations] = useState<CalculationMeropenem[]>([]);
  ...
  const addPreparation = () => {
    ...
    setSamplePreparations((current) => [
      ...current,
      createSamplePreparationMeropenem(current.length),
    ]);
    notify("MEROPENEM sample preparation added.");
  };
  ...
  const complete = () => {
    ...
    notify("MEROPENEM preparation marked as complete!");
  };
  ...
  const unlock = async () => {
    ...
    try {
      ...
      setShowUnlock(false);
      notify("MEROPENEM preparation unlocked successfully.");
    } finally {
      setUnlocking(false);
    }
  };

  const updateCalculation = (updated: CalculationMeropenem) => {
    setCalculations((current) =>
      current.map((calculation) =>
        ...
        setCalculations(
          Array.isArray(value.calculations)
            ? value.calculations.map((item, index) =>
                restoreCalculationMeropenem(item, index),
              )
            : [],
        );
        ...
        const preparations = Array.isArray(parameter.preparations)
          ? parameter.preparations
              ...
              .map((item, index) => {
                ...
                return restoreSamplePreparationMeropenem(
                  {
                    ...value,
        ...
        const worksheetCalculations = Array.isArray(parameter.calculations)
          ? parameter.calculations
              ...
              .map((item, index) => {
                ...
                return restoreCalculationMeropenem(data ?? value, index);
              })
          : [];
                ...
                return {
                  id:
                    typeof value.id === "number"
                      ...
                      : `${Date.now()}-${index}`,
                  name: String(
                    value.fileName ?? value.name ?? `MEROPENEM-${index + 1}.pdf`,
                  ),
  ...
  return (
    <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-600">
                Metal Laboratory • MEROPENEM • Sample &amp; Calculations
              </p>
            </div>
          </div>
      ...
      <SamplePreparationSection<SamplePreparationMeropenem>
        title="Sample Preparations for MEROPENEM"
        ...
        renderPreparation={(preparation, index) => (
          <MeropenemSamplePreparationCard
        ...
        <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection
              files={files}
              isLocked={preparationDataLocked}
              title="Attach MEROPENEM Preparation Files (PDF)"
              maxFiles={10}
          ...
          {completed && (
            <MeropenemCalculationSection
              ...
              onAdd={() =>
                ...
                setCalculations((current) => [
                  ...current,
                  createCalculationMeropenem(current.length),
                ])
              }
              onRemove={(id) => {
...
MeropenemPreparationModule.displayName = "MeropenemPreparationModule";

export default MeropenemPreparationModule;
