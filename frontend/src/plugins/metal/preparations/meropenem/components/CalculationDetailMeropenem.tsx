...
import type { SamplePreparationMeropenem } from "../models/SamplePreparationMeropenem";
...
const step = (preparation: SamplePreparationMeropenem | null, name: string) =>
  preparation?.steps.find((item) => item.name.trim().toLowerCase() === name.trim().toLowerCase());

interface Props {
  calculation: CalculationMeropenem;
  samplePreparations: SamplePreparationMeropenem[];
  onUpdate: (calculation: CalculationMeropenem) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailMeropenem({ calculation, samplePreparations, onUpdate, onRemove, isLocked }: Props) {
  ...
  const values = useMemo(() => {
    ...
    const labelClaim = step(selected, "Label Claim");
    return {
      ...
      v2Factor: v2?.value1 ?? "",
      v2Volume: v2?.value2 ?? "",
      labelClaim: labelClaim?.value1 ?? "",
    };
  }, [selected]);

  const requiredKeys = ["sample", "blank", "v1Factor", "v1Volume", "v2Factor", "v2Volume", "labelClaim"] as const;
  const errors = requiredKeys.filter((key) => text(values[key]).trim() === "" || numberOrNull(values[key]) === null);
  const update = <K extends keyof CalculationMeropenem>(field: K, value: CalculationMeropenem[K]) =>
    onUpdate({ ...calculation, [field]: value });

  const runCalculation = () => {
    if (isLocked || !selected || errors.length > 0) return;
    const result = calculateMeropenem(values);
    onUpdate({
      ...calculation,
      ...
      labelClaimValue: values.labelClaim,
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        <button type="button" onClick={() => setExpanded((value) => !value)} className="font-semibold">{calculation.label} · MEROPENEM</button>
          ...
          {selected && (
            <>
              <div className="rounded-lg border border-emerald-200 bg-slate-50 p-4 text-center font-mono text-xs">
                ((Sample − Blank) × V1(volume) × V2(volume) × 1000 × 1000) / (V1(factor) × V2(factor) × 10,000 × Label Claim)
              </div>
              ...
              {resultText && (
                <div className="overflow-hidden rounded-lg border-2 border-emerald-300">
                  <div className="bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-2 font-bold text-white">MEROPENEM Result</div>
