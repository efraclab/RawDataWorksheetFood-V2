...
import type {
  SodiumLactateModuleData,
  SodiumLactateModuleDraft,
} from "../models/index";
import {
...
import SodiumLactateCalculationSection from "./SodiumLactateCalculationSection";

export interface SodiumLactatePreparationModuleProps {
...
const parseSteps = (value: unknown): SamplePreparationSodiumLactateStep[] => {
  if (Array.isArray(value)) return value as SamplePreparationSodiumLactateStep[];
  if (typeof value !== "string") return [];

  try {
    const parsed = JSON.parse(value);
    return Array.isArray(parsed)
      ? (parsed as SamplePreparationSodiumLactateStep[])
      : [];
  } catch {
    return [];
  }
...
const getUnitOptions = (name: string): string[] => {
  ...
  if (name === "Sample Weight (SW1)" || name === "") return ["g", "mg", "kg"];
...
const getDefaultUnit = (name: string): string => {
  ...
  if (name === "Sample Weight (SW1)" || name === "") return "g";
...
const twoValueStep = (name: string) =>
  name === "Dilution Factor 1 (V1)" ||
  name === "Dilution Factor 2 (V2)";

const SodiumLactateStep = ({
  step,
  stepNumber,
  locked,
  onChange,
}: {
  step: SamplePreparationSodiumLactateStep;
  stepNumber: number;
  locked: boolean;
  onChange: (field: keyof SamplePreparationSodiumLactateStep, value: string) => void;
}) => {
  ...
  const isV2 = step.name === "Dilution Factor 1 (V1)";
  const isV3 = step.name === "Dilution Factor 2 (V2)";
  const isV4 = false;
  const isLabelClaim = false;
        ...
        <div className="min-w-0 flex-1">
          ...
          {(isV2 || isV3 || isV4) && (
            <p className="mt-2 text-[11px] text-slate-500">
              Excel input: <span className="font-semibold">factor × volume</span> — both values are used in the SODIUM LACTATE calculation.
            </p>
          )}
          {isLabelClaim && (
            <p className="mt-2 text-[11px] text-slate-500">
              Excel input: <span className="font-semibold">100 / claim value</span> — both values are used in the SODIUM LACTATE calculation.
            </p>
          )}
        </div>
      </div>
...
const SodiumLactateSamplePreparationCard = ({
  preparation,
  disabled,
  onRemove,
  onChange,
}: {
  preparation: SamplePreparationSodiumLactate;
  disabled: boolean;
  onRemove: () => void;
  onChange: (
    stepIndex: number,
    field: keyof SamplePreparationSodiumLactateStep,
    value: string,
  ) => void;
}) => {
  return (
    <div className="group relative z-20">
      ...
      <div className="relative mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white/95">
            ...
            <div className="flex flex-1 items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-100">
                  Sample Preparation for SODIUM LACTATE
                </p>
              </div>
            </div>
              ...
              <button
                ...
                onClick={() => {
                  const element = document.getElementById(
                    `sodiumLactate-prep-${preparation.id}`,
                  );
        ...
        <div
          id={`sodiumLactate-prep-${preparation.id}`}
...
const SodiumLactatePreparationModule = forwardRef<
  PreparationModuleHandle<SodiumLactateModuleDraft, unknown>,
  SodiumLactatePreparationModuleProps
>((props, ref) => {
  const {
    parameterId,
    parameterName,
  ...
  const [calculations, setCalculations] = useState<CalculationSodiumLactate[]>([]);
  ...
  const addPreparation = () => {
    ...
    setSamplePreparations((current) => [
      ...current,
      createSamplePreparationSodiumLactate(current.length),
    ]);
    notify("SODIUM LACTATE sample preparation added.");
  };
  ...
  const complete = () => {
    ...
    notify("SODIUM LACTATE preparation marked as complete!");
  };
  ...
  const unlock = async () => {
    ...
    try {
      ...
      setShowUnlock(false);
      notify("SODIUM LACTATE preparation unlocked successfully.");
    } finally {
      setUnlocking(false);
    }
  };

  const updateCalculation = (updated: CalculationSodiumLactate) => {
    setCalculations((current) =>
      current.map((calculation) =>
  ...
  useImperativeHandle(
    ref,
    () => ({
      getDraft: (): SodiumLactateModuleDraft => ({
        samplePreparations,
        files,
        calculations,
        completed,
      ...
      loadDraft: (draft) => {
        ...
        const value = draft as Partial<SodiumLactateModuleData>;

        setSamplePreparations(
          Array.isArray(value.samplePreparations)
            ? value.samplePreparations.map((item, index) =>
                restoreSamplePreparationSodiumLactate(item, index),
              )
            : [],
        );
        ...
        setCalculations(
          Array.isArray(value.calculations)
            ? value.calculations.map((item, index) =>
                restoreCalculationSodiumLactate(item, index),
              )
            : [],
        );
        ...
        const preparations = Array.isArray(parameter.preparations)
          ? parameter.preparations
              ...
              .map((item, index) => {
                ...
                return restoreSamplePreparationSodiumLactate(
                  {
                    ...value,
        ...
        const worksheetCalculations = Array.isArray(parameter.calculations)
          ? parameter.calculations
              ...
              .map((item, index) => {
                ...
                return restoreCalculationSodiumLactate(data ?? value, index);
              })
          : [];
                ...
                return {
                  id:
                    typeof value.id === "number"
                      ...
                      : `${Date.now()}-${index}`,
                  name: String(
                    value.fileName ?? value.name ?? `SODIUM LACTATE-${index + 1}.pdf`,
                  ),
  ...
  return (
    <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-emerald-900">
                SODIUM LACTATE Analysis
              </h2>
              <p className="text-sm text-emerald-600">
                Metal Laboratory • SODIUM LACTATE • Sample &amp; Calculations
              </p>
            </div>
          </div>
      ...
      <SamplePreparationSection<SamplePreparationSodiumLactate>
        title="Sample Preparations for SODIUM LACTATE"
        ...
        renderPreparation={(preparation, index) => (
          <SodiumLactateSamplePreparationCard
        ...
        <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection
              files={files}
              isLocked={preparationDataLocked}
              title="Attach SODIUM LACTATE Preparation Files (PDF)"
              maxFiles={10}
          ...
          <div className="mx-6">
            <PreparationCompleteSection
              preparationName="SODIUM LACTATE"
          ...
          {completed && (
            <SodiumLactateCalculationSection
              ...
              onAdd={() =>
                ...
                setCalculations((current) => [
                  ...current,
                  createCalculationSodiumLactate(current.length),
                ])
              }
              onRemove={(id) => {
...
SodiumLactatePreparationModule.displayName = "SodiumLactatePreparationModule";

export default SodiumLactatePreparationModule;
