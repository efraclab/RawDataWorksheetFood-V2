...
import TalcCalculationSection from "./TalcCalculationSection";

export interface TalcPreparationModuleProps {
...
const parseSteps = (value: unknown): SamplePreparationTalcStep[] => {
  if (Array.isArray(value)) return value as SamplePreparationTalcStep[];
  if (typeof value !== "string") return [];

  try {
    const parsed = JSON.parse(value);
    return Array.isArray(parsed)
      ? (parsed as SamplePreparationTalcStep[])
      : [];
  } catch {
    return [];
  }
...
const getUnitOptions = (name: string): string[] => {
  ...
  if (name === "Weight of Sample (SW)") return ["g", "mg", "kg"];
...
const getDefaultUnit = (name: string): string => {
  ...
  if (name === "Weight of Sample (SW)") return "g";
...
const TalcStep = ({
  ...
  step: SamplePreparationTalcStep;
  stepNumber: number;
  locked: boolean;
  onChange: (field: keyof SamplePreparationTalcStep, value: string) => void;
}) => {
        ...
        <div className="min-w-0 flex-1">
          ...
          {(isV2 || isV3 || isV4) && (
            <p className="mt-2 text-[11px] text-slate-500">
              Excel input: <span className="font-semibold">factor × volume</span> — both values are used in the TALC calculation.
            </p>
          )}
          {isLabelClaim && (
            <p className="mt-2 text-[11px] text-slate-500">
              Excel input: <span className="font-semibold">100 / claim value</span> — both values are used in the TALC calculation.
            </p>
          )}
        </div>
      </div>
...
const TalcSamplePreparationCard = ({
  preparation,
  disabled,
  onRemove,
  onChange,
}: {
  preparation: SamplePreparationTalc;
  disabled: boolean;
  onRemove: () => void;
  onChange: (
    stepIndex: number,
    field: keyof SamplePreparationTalcStep,
    value: string,
  ) => void;
}) => {
  return (
...
const TalcPreparationModule = forwardRef<
  PreparationModuleHandle<TalcModuleDraft, unknown>,
  TalcPreparationModuleProps
>((props, ref) => {
  const {
    parameterId,
    parameterName,
  ...
  const [calculations, setCalculations] = useState<CalculationTalc[]>([]);
  ...
  const addPreparation = () => {
    ...
    setSamplePreparations((current) => [
      ...current,
      createSamplePreparationTalc(current.length),
    ]);
    notify("TALC sample preparation added.");
  };
  ...
  const complete = () => {
    ...
    notify("TALC preparation marked as complete!");
  };
  ...
  const unlock = async () => {
    ...
    try {
      ...
      setShowUnlock(false);
      notify("TALC preparation unlocked successfully.");
    } finally {
      setUnlocking(false);
    }
  };

  const updateCalculation = (updated: CalculationTalc) => {
    setCalculations((current) =>
      current.map((calculation) =>
        ...
        setCalculations(
          Array.isArray(value.calculations)
            ? value.calculations.map((item, index) =>
                restoreCalculationTalc(item, index),
              )
            : [],
        );
        ...
        const preparations = Array.isArray(parameter.preparations)
          ? parameter.preparations
              ...
              .map((item, index) => {
                ...
                return restoreSamplePreparationTalc(
                  {
                    ...value,
        ...
        const worksheetCalculations = Array.isArray(parameter.calculations)
          ? parameter.calculations
              ...
              .map((item, index) => {
                ...
                return restoreCalculationTalc(data ?? value, index);
              })
          : [];
                ...
                return {
                  id:
                    typeof value.id === "number"
                      ...
                      : `${Date.now()}-${index}`,
                  name: String(
                    value.fileName ?? value.name ?? `TALC-${index + 1}.pdf`,
                  ),
  ...
  return (
    <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-600">
                Metal Laboratory • TALC • Sample &amp; Calculations
              </p>
            </div>
          </div>
      ...
      <SamplePreparationSection<SamplePreparationTalc>
        title="Sample Preparations for TALC"
        ...
        <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection
              files={files}
              isLocked={preparationDataLocked}
              title="Attach TALC Preparation Files (PDF)"
              maxFiles={10}
          ...
          {completed && (
            <TalcCalculationSection
              ...
              onAdd={() =>
                ...
                setCalculations((current) => [
                  ...current,
                  createCalculationTalc(current.length),
                ])
              }
              onRemove={(id) => {
...
TalcPreparationModule.displayName = "TalcPreparationModule";

export default TalcPreparationModule;
