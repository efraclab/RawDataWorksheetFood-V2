...
import type { SamplePreparationAnofer } from "../models/SamplePreparationAnofer";
import { calculateAnofer } from "../calculation";

export interface CalculationDetailAnoferProps {
  calculation: CalculationAnofer;
  samplePreparations: SamplePreparationAnofer[];
  onUpdate: (calculation: CalculationAnofer) => void;
  onRemove: () => void;
  isLocked: boolean;
}
...
const step = (preparation: SamplePreparationAnofer | null, name: string) =>
  preparation?.steps.find((item) => item.name === name);

const CalculationDetailAnofer: React.FC<CalculationDetailAnoferProps> = ({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const values = useMemo(() => {
    ...
    const v4 = step(selected, "Dilution Factor 3 (V4)");
    const sachet = step(selected, "Avg. Weight");
    ...
    return {
      ...
      v4Factor: v4?.value1 ?? "", v4Volume: v4?.value2 ?? "", v4VolumeUnit: v4?.unit2 ?? "ml",
      avgWeight: sachet?.value1 ?? "", avgWeightUnit: sachet?.unit1 ?? "g", molecularWeight: mw?.value1 ?? "",
  ...
  const errors = useMemo(() => {
    ...
    const required: Array<[string, unknown, boolean]> = [
      ...
      ["V4 factor", values.v4Factor, true],
      ["V4 volume", values.v4Volume, true],
      ["Avg. Weight", values.avgWeight, true],
  ...
  const update = <K extends keyof CalculationAnofer>(
    field: K,
    value: CalculationAnofer[K],
  ) => {
    onUpdate({
      ...calculation,
      [field]: value,
  ...
  const performCalculation = () => {
    ...
    const result = calculateAnofer({
      ...
      dilutionFactor3: values.v4Factor,
      dilutionVolume3: values.v4Volume,
      dilutionVolume3Unit: values.v4VolumeUnit,
      avgWeight: values.avgWeight,
      avgWeightUnit: values.avgWeightUnit,
    ...
    onUpdate({
      ...
      v4Factor: values.v4Factor,
      v4Volume: values.v4Volume,
      v4VolumeUnit: values.v4VolumeUnit,
      avgWeight: values.avgWeight,
      avgWeightUnit: values.avgWeightUnit,
      labelClaimBase: values.labelClaimBase,
      labelClaimValue: values.labelClaimValue,
    ...
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="relative bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900">
        <div className="flex items-center justify-between px-4 py-3 text-white">
          <button type="button" onClick={() => setExpanded((v) => !v)} className="flex min-w-0 flex-1 items-center gap-4 text-left">
            ...
            <span className="min-w-0"><span className="block text-sm font-semibold">{calculation.label}</span><span className="block text-xs text-emerald-100">ANOFER</span></span>
          </button>
      ...
      {expanded && <div className="space-y-6 bg-gradient-to-b from-white via-white to-slate-50 p-6">
        ...
        {selected && errors.length > 0 && <div className="rounded-lg border-2 border-red-200 bg-red-50 px-5 py-4">
          <h4 className="mb-2 text-sm font-bold text-red-800">Validation Error</h4>
              ...
              <div className="flex flex-col items-center">
                <div className="w-full border-b-2 border-black px-2 pb-3 text-center">
                  <p className="break-words text-xs font-mono leading-5 text-black">
                    Instrument Concentration (Sample) × V1 × V2(volume) × V3(volume) × V4(volume) × Avg. Weight × Label Claim Base
                  </p>
                </div>
                <div className="w-full px-2 pt-3 text-center">
                  <p className="break-words text-xs font-mono leading-5 text-black">
                    Sample Weight (SW1) × 1000 × V2(factor) × V3(factor) × V4(factor) × Label Claim Value
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded bg-emerald-50 p-4">
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-1 flex-col items-center">
                  <div className="w-full border-b-2 border-black px-2 pb-3 text-center">
                    <p className="break-words text-xs font-mono leading-5 text-black">
                      {f(values.sample)} × {f(values.v1)} × {f(values.v2Volume)} × {f(values.v3Volume)} × {f(values.v4Volume)} × {f(values.avgWeight)} × {f(values.labelClaimBase)}
                    </p>
                  </div>
                  <div className="w-full px-2 pt-3 text-center">
                    <p className="break-words text-xs font-mono leading-5 text-black">
                      {f(values.sw1)} × 1000 × {f(values.v2Factor)} × {f(values.v3Factor)} × {f(values.v4Factor)} × {f(values.labelClaimValue)}
                    </p>
                  </div>
                </div>
              </div>
        ...
        {resultText && <div className="border-t-2 border-emerald-200 pt-5">
          <div className="overflow-hidden rounded-lg border-2 border-emerald-300 bg-white shadow-lg">
            <div className="bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-2"><h6 className="text-sm font-bold text-white">ANOFER Result</h6></div>
