...
import AnoferCalculationSection from "./AnoferCalculationSection";

export interface AnoferPreparationModuleProps {
...
const parseSteps = (value: unknown): SamplePreparationAnoferStep[] => {
  if (Array.isArray(value)) return value as SamplePreparationAnoferStep[];
  if (typeof value !== "string") return [];

  try {
    const parsed = JSON.parse(value);
    return Array.isArray(parsed)
      ? (parsed as SamplePreparationAnoferStep[])
      : [];
  } catch {
    return [];
  }
...
const getUnitOptions = (name: string): string[] => {
  ...
  if (name === "Sample Weight (SW1)" || name === "Avg. Weight") return ["g", "mg", "kg"];
...
const getDefaultUnit = (name: string): string => {
  ...
  if (name === "Sample Weight (SW1)" || name === "Avg. Weight") return "g";
...
const twoValueStep = (name: string) =>
  ...
  name === "Dilution Factor 2 (V3)" ||
  name === "Dilution Factor 3 (V4)" ||
  name === "Label Claim";
...
const AnoferStep = ({
  ...
  step: SamplePreparationAnoferStep;
  stepNumber: number;
  locked: boolean;
  onChange: (field: keyof SamplePreparationAnoferStep, value: string) => void;
}) => {
  ...
  const isV4 = step.name === "Dilution Factor 3 (V4)";
        ...
        <div className="min-w-0 flex-1">
          ...
          {(isV2 || isV3 || isV4) && (
            <p className="mt-2 text-[11px] text-slate-500">
              Excel input: <span className="font-semibold">factor × volume</span> — both values are used in the ANOFER calculation.
            </p>
          )}
          {isLabelClaim && (
            <p className="mt-2 text-[11px] text-slate-500">
              Excel input: <span className="font-semibold">100 / claim value</span> — both values are used in the ANOFER calculation.
            </p>
          )}
        </div>
      </div>
...
const AnoferSamplePreparationCard = ({
  preparation,
  disabled,
  onRemove,
  onChange,
}: {
  preparation: SamplePreparationAnofer;
  disabled: boolean;
  onRemove: () => void;
  onChange: (
    stepIndex: number,
    field: keyof SamplePreparationAnoferStep,
    value: string,
  ) => void;
}) => {
  return (
...
const AnoferPreparationModule = forwardRef<
  PreparationModuleHandle<AnoferModuleDraft, unknown>,
  AnoferPreparationModuleProps
>((props, ref) => {
  const {
    parameterId,
    parameterName,
  ...
  const [calculations, setCalculations] = useState<CalculationAnofer[]>([]);
  ...
  const addPreparation = () => {
    ...
    setSamplePreparations((current) => [
      ...current,
      createSamplePreparationAnofer(current.length),
    ]);
    notify("ANOFER sample preparation added.");
  };
  ...
  const complete = () => {
    ...
    notify("ANOFER preparation marked as complete!");
  };
  ...
  const unlock = async () => {
    ...
    try {
      ...
      setShowUnlock(false);
      notify("ANOFER preparation unlocked successfully.");
    } finally {
      setUnlocking(false);
    }
  };

  const updateCalculation = (updated: CalculationAnofer) => {
    setCalculations((current) =>
      current.map((calculation) =>
        ...
        setCalculations(
          Array.isArray(value.calculations)
            ? value.calculations.map((item, index) =>
                restoreCalculationAnofer(item, index),
              )
            : [],
        );
        ...
        const preparations = Array.isArray(parameter.preparations)
          ? parameter.preparations
              ...
              .map((item, index) => {
                ...
                return restoreSamplePreparationAnofer(
                  {
                    ...value,
        ...
        const worksheetCalculations = Array.isArray(parameter.calculations)
          ? parameter.calculations
              ...
              .map((item, index) => {
                ...
                return restoreCalculationAnofer(data ?? value, index);
              })
          : [];
                ...
                return {
                  id:
                    typeof value.id === "number"
                      ...
                      : `${Date.now()}-${index}`,
                  name: String(
                    value.fileName ?? value.name ?? `ANOFER-${index + 1}.pdf`,
                  ),
  ...
  return (
    <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-600">
                Metal Laboratory • ANOFER • Sample &amp; Calculations
              </p>
            </div>
          </div>
      ...
      <SamplePreparationSection<SamplePreparationAnofer>
        title="Sample Preparations for ANOFER"
        ...
        <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection
              files={files}
              isLocked={preparationDataLocked}
              title="Attach ANOFER Preparation Files (PDF)"
              maxFiles={10}
          ...
          {completed && (
            <AnoferCalculationSection
              ...
              onAdd={() =>
                ...
                setCalculations((current) => [
                  ...current,
                  createCalculationAnofer(current.length),
                ])
              }
              onRemove={(id) => {
...
AnoferPreparationModule.displayName = "AnoferPreparationModule";

export default AnoferPreparationModule;
