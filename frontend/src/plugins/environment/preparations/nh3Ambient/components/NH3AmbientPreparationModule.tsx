...
import type { SamplePreparationNH3Ambient, SamplePreparationNH3AmbientStep } from "../models/SamplePreparationNH3Ambient";
import type { CalculationNH3Ambient } from "../models/CalculationNH3Ambient";
import { createCalculationNH3Ambient, createSamplePreparationNH3Ambient, restoreCalculationNH3Ambient } from "../factory";
import NH3AmbientCalculationSection from "./NH3AmbientCalculationSection";
import SamplePreparationDetailNH3Ambient from "./SamplePreparationDetailNH3Ambient";

type ModuleData = {
  samplePreparations: SamplePreparationNH3Ambient[];
  files: PreparationAttachedFile[];
  calculations: CalculationNH3Ambient[];
  completed: boolean;
  completedAt: string | null;
};

export interface NH3AmbientPreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationNH3Ambient => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationNH3Ambient>;
  const defaultSteps = createSamplePreparationNH3Ambient(index).steps;
  const savedSteps = Array.isArray(source.steps) ? source.steps as SamplePreparationNH3AmbientStep[] : [];
  ...
  return {
    ...createSamplePreparationNH3Ambient(index),
    ...source,
...
const NH3AmbientPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, NH3AmbientPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationNH3Ambient[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationNH3Ambient[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationNH3Ambient(items.length)]);
      notify("NH3 Ambient sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationNH3AmbientStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("NH3 Ambient preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("NH3 Ambient preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationNH3Ambient[], preparations: SamplePreparationNH3Ambient[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationNH3Ambient(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "nh3ambient")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "nh3ambient")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationNH3Ambient(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">NH3 Ambient Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • NH3 Ambient Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for NH3 Ambient"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailNH3Ambient
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "NH3Ambient"}
            />
          )}
        />
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach NH3Ambient Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
            <PreparationCompleteSection preparationName="NH3 Ambient" isCompleted={completed} isLocked={locked} completedAt={completedAt} canUnlockPreparation={canUnlockPreparation} onComplete={() => setShowComplete(true)} onUnlock={() => setShowUnlock(true)} />
          </div>
          {completed && <NH3AmbientCalculationSection calculations={calculations} samplePreparations={samplePreparations} canEditCalculations={canEditCalculations} onAdd={() => {
            setCalculations((items) => [...items, { ...createCalculationNH3Ambient(items.length), selectedSamplePreparationLabel: samplePreparations[0]?.label ?? null }]);
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="NH3 Ambient" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
NH3AmbientPreparationModule.displayName = "NH3AmbientPreparationModule";
export default NH3AmbientPreparationModule;
