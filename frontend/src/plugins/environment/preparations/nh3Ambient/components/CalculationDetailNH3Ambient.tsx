...
import type { SamplePreparationNH3Ambient } from "../models/SamplePreparationNH3Ambient";
import { calculateNH3Ambient } from "../calculation";
...
const step = (
  preparation: SamplePreparationNH3Ambient | null,
  names: string[],
) => {
...
interface Props {
  calculation: CalculationNH3Ambient;
  samplePreparations: SamplePreparationNH3Ambient[];
  onUpdate: (calculation: CalculationNH3Ambient) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailNH3Ambient({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const errors = [
    text(values.absorbanceSample).trim() === "" || numberOrNull(values.absorbanceSample) === null ? "Absorbance of sample is required and must be numeric" : "",
    text(values.absorbanceBlank).trim() === "" || numberOrNull(values.absorbanceBlank) === null ? "Absorbance of reagent blank is required and must be numeric" : "",
    text(values.calibrationFactor).trim() === "" || numberOrNull(values.calibrationFactor) === null ? "Calibration factor is required and must be numeric" : "",
  ...
  if (numberOrNull(values.absorbanceSample) !== null && numberOrNull(values.absorbanceSample)! < 0) errors.push("Absorbance of sample cannot be negative");
  if (numberOrNull(values.absorbanceBlank) !== null && numberOrNull(values.absorbanceBlank)! < 0) errors.push("Absorbance of reagent blank cannot be negative");
  if (numberOrNull(values.calibrationFactor) !== null && numberOrNull(values.calibrationFactor)! < 0) errors.push("Calibration factor cannot be negative");
  ...
  const update = <K extends keyof CalculationNH3Ambient>(
    field: K,
    value: CalculationNH3Ambient[K],
  ...
  const runCalculation = () => {
    ...
    const result = calculateNH3Ambient(values);
