...
const createStep = (name: string, unit1 = ""): SamplePreparationNH3AmbientStep => ({
  name, value1: "", unit1, logBookID: "",
});

export const createSamplePreparationNH3Ambient = (index: number): SamplePreparationNH3Ambient => ({
  id: createId(index),
...
export const createCalculationNH3Ambient = (index: number): CalculationNH3Ambient => ({
  id: createId(index),
...
export const restoreCalculationNH3Ambient = (value: unknown): CalculationNH3Ambient => {
  const source = value && typeof value === "object" && "data" in value
    ? (value as { data?: unknown }).data : value;
  const data = source && typeof source === "object" ? source as Partial<CalculationNH3Ambient> : {};
  return {
