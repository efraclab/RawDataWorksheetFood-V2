...
import { soilAvailablePhosphorousPreparationDefinition } from "./definition";

const ok = async (context: PreparationContext): Promise<PreparationResult> => ({
  success: true, data: context.data, errors: [], warnings: [],
});

export const soilAvailablePhosphorousPreparationHandler: PreparationHandler = {
  definition: soilAvailablePhosphorousPreparationDefinition,
