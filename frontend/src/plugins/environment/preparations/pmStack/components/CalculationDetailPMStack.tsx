...
import { calculatePMStack } from "../calculation";
...
const step = (preparation: SamplePreparationPMStack | null, names: string[]) => {
...
interface Props {
  calculation: CalculationPMStack;
...
export default function CalculationDetailPMStack({ calculation, samplePreparations, onUpdate, onRemove, isLocked }: Props) {
  ...
  const values = useMemo(() => {
    ...
    return {
      vstd: find(["Volume of Gas (at 250C & 760 mmHg)"]),
      w1: find(["Initial weight of Thimble"]),
      w2: find(["Final weight of Thimble"]),
    };
  }, [selected]);

  const errors = [
    text(values.vstd).trim() === "" || numberOrNull(values.vstd) === null ? "Volume of gas (at 250C & 760 mmHg) is required and must be numeric" : "",
    text(values.w1).trim() === "" || numberOrNull(values.w1) === null ? "Initial weight of thimble is required and must be numeric" : "",
    text(values.w2).trim() === "" || numberOrNull(values.w2) === null ? "Final weight of thimble is required and must be numeric" : "",
  ].filter(Boolean) as string[];
  if (numberOrNull(values.w1) !== null && numberOrNull(values.w1)! < 0) errors.push("Initial weight of thimble cannot be negative");
  if (numberOrNull(values.w2) !== null && numberOrNull(values.w2)! < 0) errors.push("Final weight of thimble cannot be negative");
  if (numberOrNull(values.vstd) !== null && numberOrNull(values.vstd)! <= 0) errors.push("Volume of gas (at 250C & 760 mmHg) must be greater than zero");
  if (numberOrNull(values.w1) !== null && numberOrNull(values.w2) !== null && numberOrNull(values.w2)! < numberOrNull(values.w1)!) errors.push("Final weight of thimble cannot be less than initial weight");

  const update = <K extends keyof CalculationPMStack>(field: K, value: CalculationPMStack[K]) => onUpdate({ ...calculation, [field]: value });

  const runCalculation = () => {
    if (!selected || errors.length > 0) return;
    const result = calculatePMStack({
      vstd: values.vstd,
      w1: values.w1,
      w2: values.w2
    });
