...
import { createCalculationPMStack, createSamplePreparationPMStack, restoreCalculationPMStack, restoreSamplePreparationPMStack } from "../factory";
import PMStackCalculationSection from "./PMStackCalculationSection";
import SamplePreparationDetailPMStack from "./SamplePreparationDetailPMStack";
...
export interface PMStackPreparationModuleProps {
...
const PMStackPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, PMStackPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationPMStack[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationPMStack[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationPMStack(items.length)]);
      notify("PM Stack sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationPMStackStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("PM Stack preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        setShowUnlock(false);
        notify("PM Stack preparation unlocked successfully.");
      } finally {
        setUnlocking(false);
      }
    };
    ...
    const withDefaultSelection = (
      items: CalculationPMStack[],
      preparations: SamplePreparationPMStack[],
    ) => items.map((item) => ({
      ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredPreparations = Array.isArray(value.samplePreparations)
          ? value.samplePreparations.map((item, index) => restoreSamplePreparationPMStack(item, index))
          : [];
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationPMStack(item))
          : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.preparationType) === normalizeType("pmStack"))
          .map((item, index) => restoreSamplePreparationPMStack({
            ...(item as object),
            steps: parseUnknown((item as Record<string, unknown>).steps),
          }, index));

        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.calculationType) === normalizeType("pmStack"))
          .map((item) => {
            const source = item as Record<string, unknown>;
            return restoreCalculationPMStack(parseUnknown(source.data));
          });
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">PM (Stack) Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • PM Stack Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for PM (Stack)"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailPMStack
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "PM Stack"
              }
            />
          )}
        />
          ...
          <>
            <div className="mx-6 mb-6">
              <FileAttachmentSection
                files={files}
                isLocked={locked}
                title="Attach PM Stack Preparation Files (PDF)"
                maxFiles={10}
            ...
            {completed && (
              <PMStackCalculationSection
                ...
                onAdd={() => setCalculations((items) => [
                  ...items,
                  {
                    ...createCalculationPMStack(items.length),
...
PMStackPreparationModule.displayName = "PMStackPreparationModule";
export default PMStackPreparationModule;
