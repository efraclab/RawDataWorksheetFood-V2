...
import type { SamplePreparationSO2Stack } from "../models/SamplePreparationSO2Stack";
import { calculateSO2Stack } from "../calculation";
...
const step = (preparation: SamplePreparationSO2Stack | null, names: string[]) => {
...
interface Props {
  calculation: CalculationSO2Stack;
  samplePreparations: SamplePreparationSO2Stack[];
  onUpdate: (calculation: CalculationSO2Stack) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailSO2Stack({ calculation, samplePreparations, onUpdate, onRemove, isLocked }: Props) {
  ...
  const values = useMemo(() => {
    ...
    return {
      v: find(["Volume of barium perchlorate titrant used for the sample"]),
      vb: find(["Volume of barium perchlorate titrant used for the blank"]),
      n: find(["Normality of barium perchlorate titrate"]),
      vao: find(["Total solution, volume of SO\u2082"]),
      va: find(["Volume of sample aliquot titrated"]),
      vn: find(["Volume of gas sampled through the dry gas meter"]),
    };
  }, [selected]);

  const errors = [
    text(values.v).trim() === "" || numberOrNull(values.v) === null ? "Volume of barium perchlorate titrant used for the sample is required and must be numeric" : "",
    text(values.vb).trim() === "" || numberOrNull(values.vb) === null ? "Volume of barium perchlorate titrant used for the blank is required and must be numeric" : "",
    text(values.n).trim() === "" || numberOrNull(values.n) === null ? "Normality of barium perchlorate titrate is required and must be numeric" : "",
    text(values.vao).trim() === "" || numberOrNull(values.vao) === null ? "Total solution, volume of SO₂ is required and must be numeric" : "",
    text(values.va).trim() === "" || numberOrNull(values.va) === null ? "Volume of sample aliquot titrated is required and must be numeric" : "",
    text(values.vn).trim() === "" || numberOrNull(values.vn) === null ? "Volume of gas sampled through the dry gas meter is required and must be numeric" : "",
  ].filter(Boolean) as string[];
  if (numberOrNull(values.v) !== null && numberOrNull(values.v)! < 0) errors.push("Volume of barium perchlorate titrant used for the sample cannot be negative");
  if (numberOrNull(values.vb) !== null && numberOrNull(values.vb)! < 0) errors.push("Volume of barium perchlorate titrant used for the blank cannot be negative");
  if (numberOrNull(values.n) !== null && numberOrNull(values.n)! < 0) errors.push("Normality of barium perchlorate titrate cannot be negative");
  if (numberOrNull(values.vao) !== null && numberOrNull(values.vao)! < 0) errors.push("Total solution, volume of SO₂ cannot be negative");
  if (numberOrNull(values.va) !== null && numberOrNull(values.va)! <= 0) errors.push("Volume of sample aliquot titrated must be greater than zero");
  if (numberOrNull(values.vn) !== null && numberOrNull(values.vn)! <= 0) errors.push("Volume of gas sampled through the dry gas meter must be greater than zero");

  const update = <K extends keyof CalculationSO2Stack>(field: K, value: CalculationSO2Stack[K]) => onUpdate({ ...calculation, [field]: value });

  const runCalculation = () => {
    if (!selected || errors.length > 0) return;
    const result = calculateSO2Stack({
      v: values.v,
      vb: values.vb,
      n: values.n,
      vao: values.vao,
