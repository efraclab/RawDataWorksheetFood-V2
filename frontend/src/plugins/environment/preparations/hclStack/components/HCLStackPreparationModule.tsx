...
import type { SamplePreparationHCLStack, SamplePreparationHCLStackStep } from "../models/SamplePreparationHCLStack";
import type { CalculationHCLStack } from "../models/CalculationHCLStack";
import { createCalculationHCLStack, createSamplePreparationHCLStack, restoreCalculationHCLStack, restoreSamplePreparationHCLStack } from "../factory";
import HCLStackCalculationSection from "./HCLStackCalculationSection";
import SamplePreparationDetailHCLStack from "./SamplePreparationDetailHCLStack";

type ModuleData = {
  samplePreparations: SamplePreparationHCLStack[];
  files: PreparationAttachedFile[];
  calculations: CalculationHCLStack[];
  completed: boolean;
  completedAt: string | null;
};

export interface HCLStackPreparationModuleProps {
...
const HCLStackPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, HCLStackPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationHCLStack[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationHCLStack[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationHCLStack(items.length)]);
      notify("HCl Stack sample preparation added.");
    };
      ...
      setCalculations((items) => items.map((item) =>
        item.selectedSamplePreparationLabel === removedLabel
          ? {
            ...
            particulate: { ...item.particulate, result: null },
            h2so4: { ...item.h2so4, result: null },
            naoh: { ...item.naoh, result: null },
          }
          : item,
      ));
    };

    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationHCLStackStep, value: string, section?: SamplePreparationHCLStackStep["section"]) => {
      if (locked) return;
      setSamplePreparations((items) => items.map((item) =>
        item.id !== preparationId ? item : {
          ...item,
          steps: item.steps.map((step) => step.name !== stepName || (section && step.section !== section) ? step : { ...step, [field]: value }),
        },
      ));
      setCalculations((items) => items.map((item) => ({
        ...item,
    ...
    const complete = () => {
      ...
      notify("HCl Stack preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        setShowUnlock(false);
        notify("HCl Stack preparation unlocked successfully.");
      } finally {
        setUnlocking(false);
      }
    };
    ...
    const withDefaultSelection = (
      items: CalculationHCLStack[],
      preparations: SamplePreparationHCLStack[],
    ) => items.map((item) => ({
      ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredPreparations = Array.isArray(value.samplePreparations)
          ? value.samplePreparations.map((item, index) => restoreSamplePreparationHCLStack(item, index))
          : [];
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationHCLStack(item))
          : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.preparationType) === normalizeType("hclStack"))
          .map((item, index) => restoreSamplePreparationHCLStack({
            ...(item as object),
            steps: parseUnknown((item as Record<string, unknown>).steps),
          }, index));

        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.calculationType) === normalizeType("hclStack"))
          .map((item) => {
            const source = item as Record<string, unknown>;
            return restoreCalculationHCLStack(parseUnknown(source.data));
          });
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">HCL Stack Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • HCl Stack Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for HCL Stack"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailHCLStack
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "HCl Stack"
              }
            />
          )}
        />
          ...
          <>
            <div className="mx-6 mb-6">
              <FileAttachmentSection
                files={files}
                isLocked={locked}
                title="Attach HCl Stack Preparation Files (PDF)"
                maxFiles={10}
            ...
            {completed && (
              <HCLStackCalculationSection
                ...
                onAdd={() => setCalculations((items) => [
                  ...items,
                  {
                    ...createCalculationHCLStack(items.length),
...
HCLStackPreparationModule.displayName = "HCLStackPreparationModule";
export default HCLStackPreparationModule;
