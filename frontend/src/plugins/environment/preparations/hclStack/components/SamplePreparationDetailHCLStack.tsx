...
interface Props {
  samplePreparation: SamplePreparationHCLStack;
  onStepChange: (samplePreparationId: number, stepName: string, field: keyof SamplePreparationHCLStackStep, newValue: string, section?: HCLStackSection) => void;
  onRemove: () => void;
  role: string;
  isLocked: boolean;
...
const sections: Array<{ key: HCLStackSection; title: string }> = [
  { key: "particulate", title: "Calculation (in particulate)" },
  { key: "h2so4", title: "Calculation (in H₂SO₄)" },
  { key: "naoh", title: "Calculation (in NaOH)" },
];

const sectionSteps = (steps: SamplePreparationHCLStackStep[], section: HCLStackSection) =>
  steps.filter((step) => step.section === section);

const SamplePreparationDetailHCLStack: React.FC<Props> = ({ samplePreparation, onStepChange, onRemove, isLocked }) => {
      ...
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-emerald-100">HCl Stack inputs</div>
          </div>
        </div>
          ...
          {sections.map((section) => (
            <div key={section.key} className="overflow-hidden rounded-xl border border-emerald-200 bg-white shadow-sm">
              ...
              <div className="space-y-3 p-4">
                {sectionSteps(samplePreparation.steps, section.key).map((step) => (
                  <div key={`${section.key}-${step.name}`} className="grid items-center gap-3 rounded-lg border border-emerald-200 bg-white p-3 md:grid-cols-[minmax(0,1fr)_220px_70px]">
                    <label className="text-sm font-semibold text-emerald-900">{step.name}</label>
                    <input
                      value={step.value1 ?? ""}
                      disabled={isLocked}
                      placeholder="Enter value"
                      onChange={(event) => onStepChange(samplePreparation.id, step.name, "value1", event.target.value, section.key)}
...
export default SamplePreparationDetailHCLStack;
