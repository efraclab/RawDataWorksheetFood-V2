...
import { moistureCoalPreparationDefinition } from "./definition";

const ok = async (context: PreparationContext): Promise<PreparationResult> => ({
  success: true, data: context.data, errors: [], warnings: [],
});

export const moistureCoalPreparationHandler: PreparationHandler = {
  definition: moistureCoalPreparationDefinition,
