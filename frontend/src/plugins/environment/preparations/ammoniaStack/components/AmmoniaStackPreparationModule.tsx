...
import { createCalculationAmmoniaStack, createSamplePreparationAmmoniaStack, restoreCalculationAmmoniaStack, restoreSamplePreparationAmmoniaStack } from "../factory";
import AmmoniaStackCalculationSection from "./AmmoniaStackCalculationSection";
import SamplePreparationDetailAmmoniaStack from "./SamplePreparationDetailAmmoniaStack";

type ModuleData = {
  samplePreparations: SamplePreparationAmmoniaStack[];
  files: PreparationAttachedFile[];
  calculations: CalculationAmmoniaStack[];
  completed: boolean;
  completedAt: string | null;
};

export interface AmmoniaStackPreparationModuleProps {
...
const AmmoniaStackPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, AmmoniaStackPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationAmmoniaStack[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationAmmoniaStack[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationAmmoniaStack(items.length)]);
      notify("Ammonia Stack sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationAmmoniaStackStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("Ammonia Stack preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        setShowUnlock(false);
        notify("Ammonia Stack preparation unlocked successfully.");
      } finally {
        setUnlocking(false);
      }
    };
    ...
    const withDefaultSelection = (
      items: CalculationAmmoniaStack[],
      preparations: SamplePreparationAmmoniaStack[],
    ) => items.map((item) => ({
      ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredPreparations = Array.isArray(value.samplePreparations)
          ? value.samplePreparations.map((item, index) => restoreSamplePreparationAmmoniaStack(item, index))
          : [];
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationAmmoniaStack(item))
          : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.preparationType) === normalizeType("ammoniaStack"))
          .map((item, index) => restoreSamplePreparationAmmoniaStack({
            ...(item as object),
            steps: parseUnknown((item as Record<string, unknown>).steps),
          }, index));

        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.calculationType) === normalizeType("ammoniaStack"))
          .map((item) => {
            const source = item as Record<string, unknown>;
            return restoreCalculationAmmoniaStack(parseUnknown(source.data));
          });
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">Ammonia (Stack) Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • Ammonia Stack Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for Ammonia (Stack)"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailAmmoniaStack
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "Ammonia Stack"
              }
            />
          )}
        />
          ...
          <>
            <div className="mx-6 mb-6">
              <FileAttachmentSection
                files={files}
                isLocked={locked}
                title="Attach Ammonia Stack Preparation Files (PDF)"
                maxFiles={10}
            ...
            {completed && (
              <AmmoniaStackCalculationSection
                ...
                onAdd={() => setCalculations((items) => [
                  ...items,
                  {
                    ...createCalculationAmmoniaStack(items.length),
...
AmmoniaStackPreparationModule.displayName = "AmmoniaStackPreparationModule";
export default AmmoniaStackPreparationModule;
