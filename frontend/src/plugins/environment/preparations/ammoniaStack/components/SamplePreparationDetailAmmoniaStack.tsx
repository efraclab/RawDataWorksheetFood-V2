...
interface Props {
  samplePreparation: SamplePreparationAmmoniaStack;
  onStepChange: (samplePreparationId: number, stepName: string, field: keyof SamplePreparationAmmoniaStackStep, newValue: string) => void;
  onRemove: () => void;
  role: string;
  isLocked: boolean;
...
const SamplePreparationDetailAmmoniaStack: React.FC<Props> = ({ samplePreparation, onStepChange, onRemove, isLocked }) => {
      ...
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-emerald-100">Ammonia Stack inputs</div>
          </div>
        </div>
...
export default SamplePreparationDetailAmmoniaStack;
