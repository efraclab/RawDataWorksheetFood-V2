...
const getStepValue = (steps: SamplePreparationPM10Step[], ...names: string[]): string => {
  const wanted = names.map(normalize);
  return steps.find((step) => wanted.includes(normalize(step.name)))?.value1 ?? "";
};

interface Props {
  samplePreparation: SamplePreparationPM10;
  onStepChange: (samplePreparationId: number, stepName: string, field: keyof SamplePreparationPM10Step, newValue: string) => void;
  onRemove: () => void;
  role: string;
  isLocked: boolean;
...
const SamplePreparationDetailPM10: React.FC<Props> = ({ samplePreparation, onStepChange, onRemove, isLocked }) => {
  ...
  const volume = averageFlowRate.trim() !== "" && samplingTime.trim() !== "" && Number.isFinite(q) && Number.isFinite(t) && q > 0 && t > 0
    ? q * t
    : null;
  const calculation = calculatePM10({ averageFlowRate, samplingTime, initialWeight, finalWeight });
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white shadow-sm">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        <div className="flex items-center gap-3">
          ...
          <div><div className="font-semibold">{samplePreparation.label}</div><div className="text-xs text-emerald-100">PM10 inputs</div></div>
        </div>
...
export default SamplePreparationDetailPM10;
