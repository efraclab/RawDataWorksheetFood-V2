...
export interface PM10CalculationInput {
...
export interface PM10CalculationResult {
  success: boolean;
  result: number | null;
  volume: number | null;
  error?: string;
 ...
 * Excel logic:
 * V (m³) = Q (m³/min) × t (min)
 * PM10 (µg/m³) = ((W2 − W1) × 1,000,000) / V
 * W1 and W2 are entered in grams.
 */
export function calculatePM10(input: PM10CalculationInput): PM10CalculationResult {
  ...
  return Number.isFinite(result)
    ...
    : { success: false, result: null, volume: null, error: "Unable to calculate PM10 result." };
}
