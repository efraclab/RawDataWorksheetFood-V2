...
import { volatileMatterPreparationDefinition } from "./definition";

const ok = async (context: PreparationContext): Promise<PreparationResult> => ({
  success: true, data: context.data, errors: [], warnings: [],
});

export const volatileMatterPreparationHandler: PreparationHandler = {
  definition: volatileMatterPreparationDefinition,
