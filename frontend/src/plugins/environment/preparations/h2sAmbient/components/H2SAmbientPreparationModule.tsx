...
import { createCalculationH2SAmbient, createSamplePreparationH2SAmbient, restoreCalculationH2SAmbient } from "../factory";
import H2SAmbientCalculationSection from "./H2SAmbientCalculationSection";
import SamplePreparationDetailH2SAmbient from "./SamplePreparationDetailH2SAmbient";

type ModuleData = {
  samplePreparations: SamplePreparationH2SAmbient[];
  files: PreparationAttachedFile[];
  calculations: CalculationH2SAmbient[];
  completed: boolean;
  completedAt: string | null;
};

export interface H2SAmbientPreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationH2SAmbient => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationH2SAmbient>;
  const defaultSteps = createSamplePreparationH2SAmbient(index).steps;
  const savedSteps = Array.isArray(source.steps) ? source.steps as SamplePreparationH2SAmbientStep[] : [];
  ...
  return {
    ...createSamplePreparationH2SAmbient(index),
    ...source,
...
const H2SAmbientPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, H2SAmbientPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationH2SAmbient[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationH2SAmbient[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationH2SAmbient(items.length)]);
      notify("H2S Ambient sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationH2SAmbientStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("H2S Ambient preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("H2S Ambient preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationH2SAmbient[], preparations: SamplePreparationH2SAmbient[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationH2SAmbient(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "h2sambient")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "h2sambient")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationH2SAmbient(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">H2S Ambient Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • H2S Ambient Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for H2S Ambient"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailH2SAmbient
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "H2SAmbient"}
            />
          )}
        />
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach H2SAmbient Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
            <PreparationCompleteSection preparationName="H2S Ambient" isCompleted={completed} isLocked={locked} completedAt={completedAt} canUnlockPreparation={canUnlockPreparation} onComplete={() => setShowComplete(true)} onUnlock={() => setShowUnlock(true)} />
          </div>
          {completed && <H2SAmbientCalculationSection calculations={calculations} samplePreparations={samplePreparations} canEditCalculations={canEditCalculations} onAdd={() => {
            setCalculations((items) => [...items, { ...createCalculationH2SAmbient(items.length), selectedSamplePreparationLabel: samplePreparations[0]?.label ?? null }]);
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="H2S Ambient" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
H2SAmbientPreparationModule.displayName = "H2SAmbientPreparationModule";
export default H2SAmbientPreparationModule;
