...
import { createCalculationO3Ambient, createSamplePreparationO3Ambient, restoreCalculationO3Ambient } from "../factory";
import O3AmbientCalculationSection from "./O3AmbientCalculationSection";
import SamplePreparationDetailO3Ambient from "./SamplePreparationDetailO3Ambient";

type ModuleData = {
  samplePreparations: SamplePreparationO3Ambient[];
  files: PreparationAttachedFile[];
  calculations: CalculationO3Ambient[];
  completed: boolean;
  completedAt: string | null;
};

export interface O3AmbientPreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationO3Ambient => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationO3Ambient>;
  const defaultSteps = createSamplePreparationO3Ambient(index).steps;
  const savedSteps = Array.isArray(source.steps) ? source.steps as SamplePreparationO3AmbientStep[] : [];
  ...
  return {
    ...createSamplePreparationO3Ambient(index),
    ...source,
...
const O3AmbientPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, O3AmbientPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationO3Ambient[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationO3Ambient[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationO3Ambient(items.length)]);
      notify("O3 Ambient sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationO3AmbientStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("O3 Ambient preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("O3 Ambient preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationO3Ambient[], preparations: SamplePreparationO3Ambient[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationO3Ambient(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "o3ambient")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "o3ambient")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationO3Ambient(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">O3 Ambient Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • O3 Ambient Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for O3 Ambient"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailO3Ambient
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "O3Ambient"}
            />
          )}
        />
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach O3Ambient Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
            <PreparationCompleteSection preparationName="O3 Ambient" isCompleted={completed} isLocked={locked} completedAt={completedAt} canUnlockPreparation={canUnlockPreparation} onComplete={() => setShowComplete(true)} onUnlock={() => setShowUnlock(true)} />
          </div>
          {completed && <O3AmbientCalculationSection calculations={calculations} samplePreparations={samplePreparations} canEditCalculations={canEditCalculations} onAdd={() => {
            setCalculations((items) => [...items, { ...createCalculationO3Ambient(items.length), selectedSamplePreparationLabel: samplePreparations[0]?.label ?? null }]);
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="O3 Ambient" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
O3AmbientPreparationModule.displayName = "O3AmbientPreparationModule";
export default O3AmbientPreparationModule;
