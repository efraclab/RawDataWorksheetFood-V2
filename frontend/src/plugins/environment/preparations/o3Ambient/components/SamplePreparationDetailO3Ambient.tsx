...
const unitForStep = (name: string): string => {
  ...
  if (normalized.includes("volumeofairsampled")) return "m³";
  return "";
};

interface Props {
  samplePreparation: SamplePreparationO3Ambient;
  onStepChange: (samplePreparationId: number, stepName: string, field: keyof SamplePreparationO3AmbientStep, newValue: string) => void;
  onRemove: () => void;
  role: string;
  isLocked: boolean;
...
const SamplePreparationDetailO3Ambient: React.FC<Props> = ({ samplePreparation, onStepChange, onRemove, isLocked }) => {
      ...
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-emerald-100">O3 Ambient inputs</div>
          </div>
        </div>
      ...
      {expanded && (
        <div className="space-y-3 bg-gradient-to-br from-emerald-50/50 to-slate-50/30 p-5">
          {samplePreparation.steps.map((step) => {
            const fixed = Boolean(step.fixed) || normalize(step.name) === "conversionfactorultou";
            return (
              <div key={step.name} className="grid items-center gap-3 rounded-lg border border-emerald-200 bg-white p-3 md:grid-cols-[minmax(0,1fr)_220px_70px]">
                ...
                <input
                  ...
                  readOnly={fixed}
                  placeholder={fixed ? "" : "Enter value"}
                  onChange={(event) => onStepChange(samplePreparation.id, step.name, "value1", event.target.value)}
                  className={`rounded-lg border border-emerald-300 px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none ${fixed ? "cursor-not-allowed bg-emerald-50 font-semibold text-emerald-900" : "bg-white"}`}
                />
...
export default SamplePreparationDetailO3Ambient;
