...
import type { SamplePreparationO3Ambient } from "../models/SamplePreparationO3Ambient";
import { calculateO3Ambient } from "../calculation";
...
const step = (
  preparation: SamplePreparationO3Ambient | null,
  names: string[],
) => {
...
interface Props {
  calculation: CalculationO3Ambient;
  samplePreparations: SamplePreparationO3Ambient[];
  onUpdate: (calculation: CalculationO3Ambient) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailO3Ambient({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const values = useMemo(() => {
    ...
    return {
      ...
      conversionFactor: find(["Conversion factor, µl to µg"]),
    };
  }, [selected]);
  ...
  const errors = requiredKeys
    ...
    .map((key) => {
      const labels: Record<(typeof requiredKeys)[number], string> = {
        absorbanceSample: "Absorbance of sample",
        absorbanceBlank: "Absorbance of reagent blank",
        calibrationFactor: "Calibration factor",
        volumeAirSampled: "Volume of air sampled",
        conversionFactor: "Conversion factor",
      };
      return `${labels[key]} is required and must be numeric`;
    })
    .concat(
      numberOrNull(values.volumeAirSampled) !== null && numberOrNull(values.volumeAirSampled)! <= 0
        ? ["Volume of air sampled must be greater than zero"]
        : [],
      numberOrNull(values.conversionFactor) !== null && numberOrNull(values.conversionFactor)! !== 1.962
        ? ["Conversion factor must be 1.962"]
        : [],
    );

  const update = <K extends keyof CalculationO3Ambient>(
    field: K,
    value: CalculationO3Ambient[K],
  ...
  const runCalculation = () => {
    ...
    const result = calculateO3Ambient(values);
