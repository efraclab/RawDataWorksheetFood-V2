...
import type { HFStackSection, SamplePreparationHFStack, SamplePreparationHFStackStep } from "../models/SamplePreparationHFStack";
import type { CalculationHFStack } from "../models/CalculationHFStack";
import { createCalculationHFStack, createSamplePreparationHFStack, restoreCalculationHFStack, restoreSamplePreparationHFStack } from "../factory";
import HFStackCalculationSection from "./HFStackCalculationSection";
import SamplePreparationDetailHFStack from "./SamplePreparationDetailHFStack";

type ModuleData = {
  samplePreparations: SamplePreparationHFStack[];
  files: PreparationAttachedFile[];
  calculations: CalculationHFStack[];
  completed: boolean;
  completedAt: string | null;
};

export interface HFStackPreparationModuleProps {
...
const HFStackPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, HFStackPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationHFStack[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationHFStack[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationHFStack(items.length)]);
      notify("HF Stack sample preparation added.");
    };
    ...
    const complete = () => {
      ...
      notify("HF Stack preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        setShowUnlock(false);
        notify("HF Stack preparation unlocked successfully.");
      } finally {
        setUnlocking(false);
      }
    };
    ...
    const withDefaultSelection = (
      items: CalculationHFStack[],
      preparations: SamplePreparationHFStack[],
    ) => items.map((item) => ({
      ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredPreparations = Array.isArray(value.samplePreparations)
          ? value.samplePreparations.map((item, index) => restoreSamplePreparationHFStack(item, index))
          : [];
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationHFStack(item))
          : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.preparationType) === normalizeType("hfStack"))
          .map((item, index) => restoreSamplePreparationHFStack({
            ...(item as object),
            steps: parseUnknown((item as Record<string, unknown>).steps),
          }, index));

        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.calculationType) === normalizeType("hfStack"))
          .map((item) => {
            const source = item as Record<string, unknown>;
            return restoreCalculationHFStack(parseUnknown(source.data));
          });
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">HF (Stack) Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • HF Stack Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for HF (Stack)"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailHFStack
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "HF Stack"
              }
            />
          )}
        />
          ...
          <>
            <div className="mx-6 mb-6">
              <FileAttachmentSection
                files={files}
                isLocked={locked}
                title="Attach HF Stack Preparation Files (PDF)"
                maxFiles={10}
            ...
            {completed && (
              <HFStackCalculationSection
                ...
                onAdd={() => setCalculations((items) => [
                  ...items,
                  {
                    ...createCalculationHFStack(items.length),
...
HFStackPreparationModule.displayName = "HFStackPreparationModule";
export default HFStackPreparationModule;
