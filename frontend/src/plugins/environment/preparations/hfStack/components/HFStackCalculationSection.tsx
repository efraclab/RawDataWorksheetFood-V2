...
import type { SamplePreparationHFStack } from "../models/SamplePreparationHFStack";
import CalculationDetailHFStack from "./CalculationDetailHFStack";
...
export default function HFStackCalculationSection({ calculations, samplePreparations, onAdd, onRemove, onUpdate }: Props) {
  return (
      ...
      <div className="mx-6 rounded-2xl border border-emerald-200 bg-white/70 p-6 shadow-lg">
        ...
        {calculations.length === 0 ? (
          <div className="rounded-xl border-2 border-dashed border-emerald-300 bg-gradient-to-br from-emerald-50 via-white to-emerald-50 py-12 text-center shadow-inner">
            ...
            <p className="mb-1 mt-3 text-base font-semibold text-emerald-800">No HF Stack calculations added yet</p>
            <p className="text-xs text-emerald-600/80">Click &quot;Add Calculation&quot; to begin</p>
          </div>
        ) : (
          <div className="space-y-6">
            {calculations.map((calculation) => (
              <CalculationDetailHFStack key={calculation.id} calculation={calculation} samplePreparations={samplePreparations} onRemove={() => onRemove(calculation.id)} onUpdate={onUpdate} isLocked={false} />
            ))}
          </div>
        )}
      </div>
