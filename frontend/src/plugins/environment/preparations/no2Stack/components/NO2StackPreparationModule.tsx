...
import { createCalculationNO2Stack, createSamplePreparationNO2Stack, restoreCalculationNO2Stack, restoreSamplePreparationNO2Stack } from "../factory";
import NO2StackCalculationSection from "./NO2StackCalculationSection";
import SamplePreparationDetailNO2Stack from "./SamplePreparationDetailNO2Stack";

type ModuleData = {
  samplePreparations: SamplePreparationNO2Stack[];
  files: PreparationAttachedFile[];
  calculations: CalculationNO2Stack[];
  completed: boolean;
  completedAt: string | null;
};

export interface NO2StackPreparationModuleProps {
...
const NO2StackPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, NO2StackPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationNO2Stack[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationNO2Stack[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationNO2Stack(items.length)]);
      notify("NO₂ Stack sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationNO2StackStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("NO₂ Stack preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        setShowUnlock(false);
        notify("NO₂ Stack preparation unlocked successfully.");
      } finally {
        setUnlocking(false);
      }
    };
    ...
    const withDefaultSelection = (
      items: CalculationNO2Stack[],
      preparations: SamplePreparationNO2Stack[],
    ) => items.map((item) => ({
      ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredPreparations = Array.isArray(value.samplePreparations)
          ? value.samplePreparations.map((item, index) => restoreSamplePreparationNO2Stack(item, index))
          : [];
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationNO2Stack(item))
          : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.preparationType) === normalizeType("no2Stack"))
          .map((item, index) => restoreSamplePreparationNO2Stack({
            ...(item as object),
            steps: parseUnknown((item as Record<string, unknown>).steps),
          }, index));

        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.calculationType) === normalizeType("no2Stack"))
          .map((item) => {
            const source = item as Record<string, unknown>;
            return restoreCalculationNO2Stack(parseUnknown(source.data));
          });
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">NO₂ (Stack) Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • NO₂ Stack Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for NO₂ (Stack)"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailNO2Stack
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "NO₂ Stack"
              }
            />
          )}
        />
          ...
          <>
            <div className="mx-6 mb-6">
              <FileAttachmentSection
                files={files}
                isLocked={locked}
                title="Attach NO₂ Stack Preparation Files (PDF)"
                maxFiles={10}
            ...
            {completed && (
              <NO2StackCalculationSection
                ...
                onAdd={() => setCalculations((items) => [
                  ...items,
                  {
                    ...createCalculationNO2Stack(items.length),
...
NO2StackPreparationModule.displayName = "NO2StackPreparationModule";
export default NO2StackPreparationModule;
