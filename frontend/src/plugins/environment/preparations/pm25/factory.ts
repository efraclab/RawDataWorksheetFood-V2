...
const createStep = (name: string, unit1 = ""): SamplePreparationPM25Step => ({
  name, value1: "", unit1, logBookID: "",
});

export const createSamplePreparationPM25 = (index: number): SamplePreparationPM25 => ({
  id: createId(index),
  label: `Sample Preparation ${index + 1}`,
  steps: [
    createStep("Filter Paper Number"),
    createStep("Average Flow Rate"),
    createStep("Total Sampling Time"),
    createStep("Initial weight of Filter"),
    createStep("Final weight of Filter"),
  ],
});

export const createCalculationPM25 = (index: number): CalculationPM25 => ({
  id: createId(index),
...
export const restoreCalculationPM25 = (value: unknown): CalculationPM25 => {
  const source = value && typeof value === "object" && "data" in value
    ? (value as { data?: unknown }).data : value;
  const data = source && typeof source === "object" ? source as Partial<CalculationPM25> : {};
  return {
