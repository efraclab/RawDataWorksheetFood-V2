...
import type { SamplePreparationPM25 } from "../models/SamplePreparationPM25";
...
const step = (
  preparation: SamplePreparationPM25 | null,
  names: string[],
) => {
...
interface Props {
  calculation: CalculationPM25;
  samplePreparations: SamplePreparationPM25[];
  onUpdate: (calculation: CalculationPM25) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailPM25({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const update = <K extends keyof CalculationPM25>(
    field: K,
    value: CalculationPM25[K],
  ...
  const runCalculation = () => {
    ...
    const result = calculatePM25(values);
