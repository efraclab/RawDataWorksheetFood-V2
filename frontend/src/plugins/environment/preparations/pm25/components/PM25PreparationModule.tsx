...
import type { CalculationPM25 } from "../models/CalculationPM25";
import { createCalculationPM25, createSamplePreparationPM25, restoreCalculationPM25 } from "../factory";
import PM25CalculationSection from "./PM25CalculationSection";
import SamplePreparationDetailPM25 from "./SamplePreparationDetailPM25";

type ModuleData = {
  samplePreparations: SamplePreparationPM25[];
  files: PreparationAttachedFile[];
  calculations: CalculationPM25[];
  completed: boolean;
  completedAt: string | null;
};

export interface PM25PreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationPM25 => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationPM25>;
  const defaultSteps = createSamplePreparationPM25(index).steps;
  const savedSteps = Array.isArray(source.steps) ? source.steps as SamplePreparationPM25Step[] : [];
  ...
  return {
    ...createSamplePreparationPM25(index),
    ...source,
...
const PM25PreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, PM25PreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationPM25[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationPM25[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationPM25(items.length)]);
      notify("PM25 sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationPM25Step, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("PM25 preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("PM25 preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationPM25[], preparations: SamplePreparationPM25[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationPM25(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "pm25")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "pm25")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationPM25(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">PM25 Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • PM25 Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for PM25"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailPM25
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "PM25"}
            />
          )}
        />
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach PM25 Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
            <PreparationCompleteSection preparationName="PM25" isCompleted={completed} isLocked={locked} completedAt={completedAt} canUnlockPreparation={canUnlockPreparation} onComplete={() => setShowComplete(true)} onUnlock={() => setShowUnlock(true)} />
          </div>
          {completed && <PM25CalculationSection calculations={calculations} samplePreparations={samplePreparations} canEditCalculations={canEditCalculations} onAdd={() => {
            setCalculations((items) => [...items, { ...createCalculationPM25(items.length), selectedSamplePreparationLabel: samplePreparations[0]?.label ?? null }]);
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="PM25" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
PM25PreparationModule.displayName = "PM25PreparationModule";
export default PM25PreparationModule;
