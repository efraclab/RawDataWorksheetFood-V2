...
import type { SamplePreparationChlorineAmbient } from "../models/SamplePreparationChlorineAmbient";
import { calculateChlorineAmbient } from "../calculation";
...
const step = (
  preparation: SamplePreparationChlorineAmbient | null,
  names: string[],
) => {
...
interface Props {
  calculation: CalculationChlorineAmbient;
  samplePreparations: SamplePreparationChlorineAmbient[];
  onUpdate: (calculation: CalculationChlorineAmbient) => void;
  onRemove: () => void;
  isLocked: boolean;
}

export default function CalculationDetailChlorineAmbient({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const values = useMemo(() => {
    ...
    return {
      amountChlorineFound: find(["Amount of chlorine found"]),
      volumeAirSampled: find(["Volume of air sampled"]),
    };
  }, [selected]);

  const errors = [
    text(values.amountChlorineFound).trim() === "" || numberOrNull(values.amountChlorineFound) === null ? "Amount of chlorine found is required and must be numeric" : "",
  ...
  if (numberOrNull(values.amountChlorineFound) !== null && numberOrNull(values.amountChlorineFound)! < 0) errors.push("Amount of chlorine found cannot be negative");
  ...
  const update = <K extends keyof CalculationChlorineAmbient>(
    field: K,
    value: CalculationChlorineAmbient[K],
  ...
  const runCalculation = () => {
    ...
    const result = calculateChlorineAmbient(values);
  ...
  return (
    <div className="overflow-hidden rounded-xl border-2 border-emerald-200 bg-white shadow-lg">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        ...
        >
          {calculation.label} · Chlorine Ambient
        </button>
          ...
          {selected && (
            <>
              <div className="rounded-lg border border-emerald-200 bg-slate-50 p-4 text-center font-mono text-xs">
                Chlorine concentration (µg/m³) = M / V
              </div>
