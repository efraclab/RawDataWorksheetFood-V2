...
interface Props {
  samplePreparation: SamplePreparationChlorineAmbient;
  onStepChange: (samplePreparationId: number, stepName: string, field: keyof SamplePreparationChlorineAmbientStep, newValue: string) => void;
  onRemove: () => void;
  role: string;
  isLocked: boolean;
...
const SamplePreparationDetailChlorineAmbient: React.FC<Props> = ({ samplePreparation, onStepChange, onRemove, isLocked }) => {
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-4 overflow-hidden rounded-lg border border-emerald-200 bg-white shadow-sm">
      <div className="flex items-center justify-between bg-gradient-to-r from-emerald-700 via-emerald-800 to-slate-900 px-4 py-3 text-white">
        <div className="flex items-center gap-3">
          ...
          <div><div className="font-semibold">{samplePreparation.label}</div><div className="text-xs text-emerald-100">Chlorine Ambient inputs</div></div>
        </div>
...
export default SamplePreparationDetailChlorineAmbient;
