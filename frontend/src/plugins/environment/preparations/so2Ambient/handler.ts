...
import { so2AmbientPreparationDefinition } from "./definition";

const ok = async (context: PreparationContext): Promise<PreparationResult> => ({
  success: true, data: context.data, errors: [], warnings: [],
});

export const so2AmbientPreparationHandler: PreparationHandler = {
  definition: so2AmbientPreparationDefinition,
