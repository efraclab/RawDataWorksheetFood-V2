...
import { createCalculationSO2Ambient, createSamplePreparationSO2Ambient, restoreCalculationSO2Ambient } from "../factory";
import SO2AmbientCalculationSection from "./SO2AmbientCalculationSection";
import SamplePreparationDetailSO2Ambient from "./SamplePreparationDetailSO2Ambient";

type ModuleData = {
  samplePreparations: SamplePreparationSO2Ambient[];
  files: PreparationAttachedFile[];
  calculations: CalculationSO2Ambient[];
  completed: boolean;
  completedAt: string | null;
};

export interface SO2AmbientPreparationModuleProps {
...
const restorePreparation = (value: unknown, index: number): SamplePreparationSO2Ambient => {
  const source = (value && typeof value === "object" ? value : {}) as Partial<SamplePreparationSO2Ambient>;
  const defaultSteps = createSamplePreparationSO2Ambient(index).steps;
  const savedSteps = Array.isArray(source.steps) ? source.steps as SamplePreparationSO2AmbientStep[] : [];
  ...
  return {
    ...createSamplePreparationSO2Ambient(index),
    ...source,
...
const SO2AmbientPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, SO2AmbientPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationSO2Ambient[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationSO2Ambient[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationSO2Ambient(items.length)]);
      notify("SO2 Ambient sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationSO2AmbientStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("SO2 Ambient preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        notify("SO2 Ambient preparation unlocked successfully.");
      } finally { setUnlocking(false); }
    };

    const withDefaultSelection = (items: CalculationSO2Ambient[], preparations: SamplePreparationSO2Ambient[]) =>
      items.map((item) => ({
        ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationSO2Ambient(item)) : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.preparationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "so2ambient")
        ...
        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => String((item as Record<string, unknown>)?.calculationType ?? "").toLowerCase().replace(/[^a-z0-9]/g, "") === "so2ambient")
          .map((item) => {
            const source = item as Record<string, unknown>;
            const data = parseUnknown(source.data);
            return restoreCalculationSO2Ambient(data);
          });
        setSamplePreparations(restoredPreparations);
        setCalculations(withDefaultSelection(restoredCalculations, restoredPreparations));
        setFiles([]);
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">SO2 Ambient Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • SO2 Ambient Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for SO2 Ambient"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailSO2Ambient
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "SO2Ambient"}
            />
          )}
        />
        ...
        {samplePreparations.length > 0 && <>
          <div className="mx-6 mb-6">
            <FileAttachmentSection files={files} isLocked={locked} title="Attach SO2Ambient Preparation Files (PDF)" maxFiles={10} onAttachFiles={addFiles} onRemoveFile={(_, index) => setFiles((items) => items.filter((__, i) => i !== index))} />
          </div>
          <div className="mx-6">
            <PreparationCompleteSection preparationName="SO2 Ambient" isCompleted={completed} isLocked={locked} completedAt={completedAt} canUnlockPreparation={canUnlockPreparation} onComplete={() => setShowComplete(true)} onUnlock={() => setShowUnlock(true)} />
          </div>
          {completed && <SO2AmbientCalculationSection calculations={calculations} samplePreparations={samplePreparations} canEditCalculations={canEditCalculations} onAdd={() => {
            setCalculations((items) => [...items, { ...createCalculationSO2Ambient(items.length), selectedSamplePreparationLabel: samplePreparations[0]?.label ?? null }]);
        ...
        <PreparationCompleteModal isOpen={showComplete} preparationName="SO2 Ambient" parameterName={parameterName} parameterCode={parameterCode} onConfirm={complete} onCancel={() => setShowComplete(false)} />
...
SO2AmbientPreparationModule.displayName = "SO2AmbientPreparationModule";
export default SO2AmbientPreparationModule;
