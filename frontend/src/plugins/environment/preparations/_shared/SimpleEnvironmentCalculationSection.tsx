...
import SimpleEnvironmentCalculationDetail from "./SimpleEnvironmentCalculationDetail";
...
export default function SimpleEnvironmentCalculationSection({
  config, calculations, samplePreparations, canEditCalculations, onAdd, onRemove, onUpdate,
}: Props) {
  return (
      ...
      <div className="mx-6 rounded-2xl border border-emerald-200 bg-white/70 p-6 shadow-lg">
        <div className="mb-6 flex items-center justify-between">
          <h3 className="flex items-center gap-3 text-lg font-bold text-emerald-900">
            ...
            {config.shortName} Calculation
          </h3>
          <button type="button" onClick={onAdd} disabled={!canEditCalculations} className="flex items-center gap-2 rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white shadow-md transition-all hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-40">
        ...
        {calculations.length === 0 ? (
          <div className="rounded-xl border-2 border-dashed border-emerald-300 bg-gradient-to-br from-emerald-50 via-white to-emerald-50 py-12 text-center shadow-inner">
            ...
            <p className="mb-1 mt-3 text-base font-semibold text-emerald-800">No {config.shortName} calculations added yet</p>
