...
import { createCalculationCarbonDisulphideAmbient, createSamplePreparationCarbonDisulphideAmbient, restoreCalculationCarbonDisulphideAmbient, restoreSamplePreparationCarbonDisulphideAmbient } from "../factory";
import CarbonDisulphideAmbientCalculationSection from "./CarbonDisulphideAmbientCalculationSection";
import SamplePreparationDetailCarbonDisulphideAmbient from "./SamplePreparationDetailCarbonDisulphideAmbient";

type ModuleData = {
  samplePreparations: SamplePreparationCarbonDisulphideAmbient[];
  files: PreparationAttachedFile[];
  calculations: CalculationCarbonDisulphideAmbient[];
  completed: boolean;
  completedAt: string | null;
};

export interface CarbonDisulphideAmbientPreparationModuleProps {
...
const CarbonDisulphideAmbientPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, CarbonDisulphideAmbientPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationCarbonDisulphideAmbient[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationCarbonDisulphideAmbient[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationCarbonDisulphideAmbient(items.length)]);
      notify("Carbon Disulphide Ambient sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationCarbonDisulphideAmbientStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("Carbon Disulphide Ambient preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        setShowUnlock(false);
        notify("Carbon Disulphide Ambient preparation unlocked successfully.");
      } finally {
        setUnlocking(false);
      }
    };
    ...
    const withDefaultSelection = (
      items: CalculationCarbonDisulphideAmbient[],
      preparations: SamplePreparationCarbonDisulphideAmbient[],
    ) => items.map((item) => ({
      ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredPreparations = Array.isArray(value.samplePreparations)
          ? value.samplePreparations.map((item, index) => restoreSamplePreparationCarbonDisulphideAmbient(item, index))
          : [];
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationCarbonDisulphideAmbient(item))
          : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.preparationType) === normalizeType("carbonDisulphideAmbient"))
          .map((item, index) => restoreSamplePreparationCarbonDisulphideAmbient({
            ...(item as object),
            steps: parseUnknown((item as Record<string, unknown>).steps),
          }, index));

        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.calculationType) === normalizeType("carbonDisulphideAmbient"))
          .map((item) => {
            const source = item as Record<string, unknown>;
            return restoreCalculationCarbonDisulphideAmbient(parseUnknown(source.data));
          });
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">Carbon Disulphide (Ambient) Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • Carbon Disulphide Ambient Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for Carbon Disulphide (Ambient)"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailCarbonDisulphideAmbient
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "Carbon Disulphide Ambient"
              }
            />
          )}
        />
          ...
          <>
            <div className="mx-6 mb-6">
              <FileAttachmentSection
                files={files}
                isLocked={locked}
                title="Attach Carbon Disulphide Ambient Preparation Files (PDF)"
                maxFiles={10}
            ...
            <div className="mx-6">
              <PreparationCompleteSection
                preparationName="Carbon Disulphide (Ambient)"
            ...
            {completed && (
              <CarbonDisulphideAmbientCalculationSection
                ...
                onAdd={() => setCalculations((items) => [
                  ...items,
                  {
                    ...createCalculationCarbonDisulphideAmbient(items.length),
...
CarbonDisulphideAmbientPreparationModule.displayName = "CarbonDisulphideAmbientPreparationModule";
export default CarbonDisulphideAmbientPreparationModule;
