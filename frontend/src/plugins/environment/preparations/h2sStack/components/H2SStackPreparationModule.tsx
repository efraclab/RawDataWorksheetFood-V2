...
import { createCalculationH2SStack, createSamplePreparationH2SStack, restoreCalculationH2SStack, restoreSamplePreparationH2SStack } from "../factory";
import H2SStackCalculationSection from "./H2SStackCalculationSection";
import SamplePreparationDetailH2SStack from "./SamplePreparationDetailH2SStack";

type ModuleData = {
  samplePreparations: SamplePreparationH2SStack[];
  files: PreparationAttachedFile[];
  calculations: CalculationH2SStack[];
  completed: boolean;
  completedAt: string | null;
};

export interface H2SStackPreparationModuleProps {
...
const H2SStackPreparationModule = forwardRef<PreparationModuleHandle<ModuleData, unknown>, H2SStackPreparationModuleProps>(
  (props, ref) => {
    const {
    ...
    const [samplePreparations, setSamplePreparations] = useState<SamplePreparationH2SStack[]>([]);
    const [files, setFiles] = useState<PreparationAttachedFile[]>([]);
    const [calculations, setCalculations] = useState<CalculationH2SStack[]>([]);
    ...
    const addPreparation = () => {
      ...
      setSamplePreparations((items) => [...items, createSamplePreparationH2SStack(items.length)]);
      notify("H₂S in Stack sample preparation added.");
    };
    ...
    const updateStep = (preparationId: number, stepName: string, field: keyof SamplePreparationH2SStackStep, value: string) => {
      if (locked) return;
    ...
    const complete = () => {
      ...
      notify("H₂S in Stack preparation marked as complete.");
    };
    ...
    const unlock = async () => {
      ...
      try {
        ...
        setShowUnlock(false);
        notify("H₂S in Stack preparation unlocked successfully.");
      } finally {
        setUnlocking(false);
      }
    };
    ...
    const withDefaultSelection = (
      items: CalculationH2SStack[],
      preparations: SamplePreparationH2SStack[],
    ) => items.map((item) => ({
      ...item,
    ...
    useImperativeHandle(ref, () => ({
      ...
      loadDraft: (draft) => {
        ...
        const restoredPreparations = Array.isArray(value.samplePreparations)
          ? value.samplePreparations.map((item, index) => restoreSamplePreparationH2SStack(item, index))
          : [];
        const restoredCalculations = Array.isArray(value.calculations)
          ? value.calculations.map((item) => restoreCalculationH2SStack(item))
          : [];
      ...
      restoreFromWorksheet: async (worksheet) => {
        ...
        const restoredPreparations = (value.preparations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.preparationType) === normalizeType("h2sStack"))
          .map((item, index) => restoreSamplePreparationH2SStack({
            ...(item as object),
            steps: parseUnknown((item as Record<string, unknown>).steps),
          }, index));

        const restoredCalculations = (value.calculations ?? [])
          .filter((item) => normalizeType((item as Record<string, unknown>)?.calculationType) === normalizeType("h2sStack"))
          .map((item) => {
            const source = item as Record<string, unknown>;
            return restoreCalculationH2SStack(parseUnknown(source.data));
          });
    ...
    return (
      <div className="mt-8 overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-white via-emerald-50/30 to-white pb-8 shadow-2xl">
        <div className="bg-gradient-to-r from-white via-white to-emerald-50 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-emerald-900">H₂S in Stack Analysis</h2>
                <p className="text-sm text-emerald-600">Environment Laboratory • H₂S in Stack Testing</p>
              </div>
            </div>
        ...
        <SamplePreparationSection
          title="Sample Preparations for H₂S in Stack"
          ...
          renderPreparation={(preparation, index) => (
            <SamplePreparationDetailH2SStack
              samplePreparation={preparation}
              onStepChange={updateStep}
              onRemove={() => removePreparation(index)}
              role={props.role}
              isLocked={locked}
              parameterType={parameterCode ?? parameterName ?? "H₂S in Stack"
              }
            />
          )}
        />
          ...
          <>
            <div className="mx-6 mb-6">
              <FileAttachmentSection
                files={files}
                isLocked={locked}
                title="Attach H₂S in Stack Preparation Files (PDF)"
                maxFiles={10}
            ...
            {completed && (
              <H2SStackCalculationSection
                ...
                onAdd={() => setCalculations((items) => [
                  ...items,
                  {
                    ...createCalculationH2SStack(items.length),
...
H2SStackPreparationModule.displayName = "H2SStackPreparationModule";
export default H2SStackPreparationModule;
