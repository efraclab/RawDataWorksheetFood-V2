...
import { acidvaluePreparationDefinition } from "./definition";

export const acidvaluePreparationHandler: PreparationHandler = {
  definition: acidvaluePreparationDefinition,
  capabilities: {
    ...
    validate: async (context: PreparationContext): Promise<PreparationResult> => {
      ...
      const errors = hasPreparation ? [] : ["Acid Value Sample Preparation is required."];
