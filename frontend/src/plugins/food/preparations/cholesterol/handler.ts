...
import { cholesterolPreparationDefinition } from "./definition";

export const cholesterolPreparationHandler: PreparationHandler = {
  definition: cholesterolPreparationDefinition,
  capabilities: {
    ...
    validate: async (context: PreparationContext): Promise<PreparationResult> => {
      ...
      const errors = hasPreparation ? [] : ["Cholesterol Sample Preparation is required."];
