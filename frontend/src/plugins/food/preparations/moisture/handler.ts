...
import { moisturePreparationDefinition } from "./definition";

export const moisturePreparationHandler: PreparationHandler = {
  definition: moisturePreparationDefinition,
  capabilities: {
    ...
    validate: async (context: PreparationContext): Promise<PreparationResult> => {
      ...
      const errors = hasPreparation ? [] : ["Moisture Sample Preparation is required."];
