...
import { uricacidPreparationDefinition } from "./definition";

export const uricacidPreparationHandler: PreparationHandler = {
  definition: uricacidPreparationDefinition,
  capabilities: {
    ...
    validate: async (context: PreparationContext): Promise<PreparationResult> => {
      ...
      const errors = hasPreparation ? [] : ["Uric Acid Sample Preparation is required."];
