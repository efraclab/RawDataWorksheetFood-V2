...
import { aminoacidPreparationDefinition } from "./definition";

export const aminoacidPreparationHandler: PreparationHandler = {
  definition: aminoacidPreparationDefinition,
  capabilities: {
    ...
    validate: async (context: PreparationContext): Promise<PreparationResult> => {
      ...
      const errors = hasPreparation ? [] : ["Amino Acid Sample Preparation is required."];
