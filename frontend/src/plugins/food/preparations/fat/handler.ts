...
import { fatPreparationDefinition } from "./definition";

export const fatPreparationHandler: PreparationHandler = {
  definition: fatPreparationDefinition,
  capabilities: {
    ...
    validate: async (context: PreparationContext): Promise<PreparationResult> => {
      ...
      const errors = hasPreparation ? [] : ["FAT Sample Preparation is required."];
