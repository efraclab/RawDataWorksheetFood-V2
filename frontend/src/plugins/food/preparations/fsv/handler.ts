...
import { fsvPreparationDefinition } from "./definition";

export const fsvPreparationHandler: PreparationHandler = {
  definition: fsvPreparationDefinition,
  capabilities: {
    ...
    validate: async (context: PreparationContext): Promise<PreparationResult> => {
      ...
      const errors = hasPreparation ? [] : ["FSV (A, D, E, K) Sample Preparation is required."];
