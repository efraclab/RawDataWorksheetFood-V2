...
import { sugarPreparationDefinition } from "./definition";

export const sugarPreparationHandler: PreparationHandler = {
  definition: sugarPreparationDefinition,
  capabilities: {
    ...
    validate: async (context: PreparationContext): Promise<PreparationResult> => {
      ...
      const errors = hasPreparation ? [] : ["Total Sugar Sample Preparation is required."];
