...
import { preservativePreparationDefinition } from "./definition";

export const preservativePreparationHandler: PreparationHandler = {
  definition: preservativePreparationDefinition,
  capabilities: {
    ...
    validate: async (context: PreparationContext): Promise<PreparationResult> => {
      ...
      const errors = hasPreparation ? [] : ["Preservative Sample Preparation is required."];
