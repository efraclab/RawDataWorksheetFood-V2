...
import { unsapmatterPreparationDefinition } from "./definition";

export const unsapmatterPreparationHandler: PreparationHandler = {
  definition: unsapmatterPreparationDefinition,
  capabilities: {
    ...
    validate: async (context: PreparationContext): Promise<PreparationResult> => {
      ...
      const errors = hasPreparation ? [] : ["Unsaponifiable Matter Sample Preparation is required."];
